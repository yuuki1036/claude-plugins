...
| [code-review](#code-review) | 2.131.1 | 5 | 5 | - | SessionStart, Stop, PreToolUse | - | Phase 0 トリアージ + 動的構成コードレビュー |
| [design-doc](#design-doc) | 0.5.3 | 2 | 2 | 1 | - | - | 技術設計書を実装に入らず作成・永続化 + 4視点レビュー |
| [dev-workflow](#dev-workflow) | 1.34.9 | 4 | 7 | - | Pre/PostToolUse, SessionStart | ✓ | Git コミット・PR・UI 確認・バグ診断・worktree |
| [doc-freshness](#doc-freshness) | 0.5.3 | 1 | 1 | - | PostToolUse, SessionStart | - | frontmatter による doc 鮮度機械強制 |
| [failure-journal](#failure-journal) | 0.7.0 | 2 | 2 | - | SessionStart, PostCompact | - | 再発失敗の fingerprint 集計・retro 還流 |
| [feature-dev](#feature-dev) | 2.17.7 | 1 | 1 | 2 | SessionStart | - | 8 phase 機能開発ワークフロー |
| [guardrail-protect](#guardrail-protect) | 0.5.2 | - | - | - | PreToolUse | - | 設定骨抜き・--no-verify・実在しない見出し参照・隔離なしの hook 実行を機械ブロック |
| [issue-workflow](#issue-workflow) | 1.7.7 | 13 | 13 | 4 | 5 events | - | Issue 管理（linear/indie 統合後継・backend 自動判定） |
...
### doc-freshness
ドキュメント鮮度の機械強制。last-validated / phase frontmatter による stale 検出、行数ガード、internal link 検証、新規 doc grace period。手動走査（skill）に加え、PostToolUse hook で frontmatter 必須の project doc（既定は .claude/designs・.claude/adr・.claude/living-specs）への frontmatter 欠落を非ブロッキング検知、SessionStart hook（opt-in）で stale を一括通知。
...
| `follow-up` | `{DATA_DIR}/{slug}/follow-ups/*.md` | issue-workflow |
| `knowledge` | `{DATA_DIR}/{slug}/knowledge/**/*.md` | issue-workflow |
