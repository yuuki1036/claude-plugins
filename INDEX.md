...
| [dev-workflow](#dev-workflow) | 1.22.0 | 3 | 5 | - | Pre/PostToolUse, SessionStart | ✓ | Git コミット・PR・UI 確認・worktree |
...
| [indie-workflow](#indie-workflow) | 1.32.2 | 10 | 10 | 2 | 5 events | - | 個人開発向けローカル Issue 管理 |
| [linear-workflow](#linear-workflow) | 1.31.2 | 10 | 10 | 3 | 4 events | - | Linear MCP 連携の Issue/プロジェクト管理 |
...
| [writing-polish](#writing-polish) | 0.6.0 | 1 | 1 | - | - | - | 文章を語句レベルで推敲・添削 |
...
### code-review
Phase 0 トリアージ + 動的エージェント構成のコードレビュー。confidence × severity 2 軸スコアリング、red-flag specialist 自動起動、high severity 検出時の meta-reviewer ラウンド、反証レイヤー（Phase 5.8/4.8: 指摘を独立エージェントが反証、高 severity は消さず係争注記、specialist 除外）。事実主張のツール接地 (claim grounding) と over-correction ガード（issue #71）。self-review は `--embed` で他プラグインから委譲可能。
