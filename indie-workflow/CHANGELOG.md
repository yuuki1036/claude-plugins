...
### Added
- **indie-issue-maintain に未import knowledge 検知ステップを追加**（issue #86）。knowledge 切り出し完了後に、横断 vault（別リポジトリ）への未反映件数を検知して `/import-knowledge` を促す。切り出し直後が発火位置として最も的確（切り出さない＝新規 import も無い、という論理も噛み合い、セッション開始 hook より発火位置が正確）:
