...
### Changed
- `issue-design` / `indie-issue-discover` skill の description を圧縮（常駐コンテキスト削減）。責務分離文・「起動＝実行確定」の説明・連携先の内部機構を SKILL.md 本文（責務境界表・設計原則節）へ降ろし、what + トリガーに絞った。`issue-design` は linear-workflow 1.37.2 とミラー対称に削減（eval pass^k=3 で非退行を確認）。

## [1.40.1] - 2026-07-16

### Fixed
- **並列起動した agent の結果取りこぼしを修正**（linear-workflow 1.37.1 とミラー）。CC 2.1.198 で Agent tool の既定が background 実行に変わったため、indie-start Phase F3.5 の Context Recovery Agent Team と indie-issue-discover の観点スキャン Agent / discover-verifier に `run_in_background: false` の明示を追加（省略すると完了を待たずに進み結果を取りこぼす）

## [1.40.0] - 2026-07-13

### Added
- **indie-issue-maintain に未import knowledge の検知→vault 反映促しを追加**（issue #86）。knowledge の切り出しはこのスキルの責務だが、横断 vault へ反映する `/import-knowledge` は手動トリガーで忘れがち。切り出し完了直後（処理フロー 13.5）に未import件数を検知し、N>0 なら `/import-knowledge {slug}` を最終レポートで促す:
  - 検知は vault 側の軽量 CLI `unimported_scan.py`（`$KNOWLEDGE_VAULT_ROOT/_shared/scripts/`）に委譲。`--project` で cwd プロジェクトに絞り `--count` で件数のみ取得（ベクトル検索を使わず軽量）
  - **feature-dev Phase 1.6 の detect→skip パターンを踏襲**。`KNOWLEDGE_VAULT_ROOT` 未設定 or scan スクリプト不在なら後方互換で skip（vault を持たないマシンで壊れない）
  - 促しは非ブロッキングの推奨に留め、fail-silent（N==0 / skip / エラー時は何も出さない）
