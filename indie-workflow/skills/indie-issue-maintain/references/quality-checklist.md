...
`/indie-issue-maintain` で Issue を `completed` に遷移させる前、または完了サブタスクが多い場合、コードレビューが実施されたかを確認する。feature-dev 経由を通らないケースでのレビュー素通りを防ぐ。
...
Issue ファイルの「スコープ外」「後続 Issue 候補」「やらないこと」セクションの**前回コミット以降の追加行**を検出し、`/indie-follow-up new` 候補として提示する。
