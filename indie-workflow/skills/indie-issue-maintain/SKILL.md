...
フロントマターの `scope_size` と実際のタスク数（進捗セクション内のチェックリスト項目数）を比較し、閾値を超過している場合は警告する。
...
**警告フォーマット:**
```
WARNING: スコープ超過 - scope_size: {size}（上限 {limit}個）に対してタスク {actual}個。
Issue の分割を検討してください。
```

- 閾値超過時はユーザーに Issue 分割を提案する
- `scope_size` が未設定の場合はチェックをスキップする
...
9. タスク完了時フローの適用判定（全タスク完了 → status 更新、follow_up 確認）
10. 整理計画をユーザーに提示:
    - スコープ超過警告（該当する場合）
    ...
    - completed ファイルの削除候補
11. 承認を得てから実行
12. knowledge/ 切り出しがあった場合、knowledge/index.md を更新
13. 更新履歴にメンテナンス内容を記録
```
