...
- **eval が測っているのは「起動に使われる側」の description であって `SKILL.md` とは限らない**: command 名と skill 名が同名なら、router に載るのは `commands/*.md` の description で `SKILL.md` 側は載らない（GitHub issue #206）。**fail の原因を SKILL.md の description に求める前に、router 本人に引用させて確かめること** — `claude -p '<plugin>:<skill> について、あなたに見えている description を一字一句そのまま引用して' --permission-mode plan`。実例: `issue-create-linear-import` は SKILL.md に逐語トリガーがあるのに 6/6 で落ちていた。そのトリガーは視界に無く、commands 側の description（Linear 要素ゼロ）で判定されていた（#205）
