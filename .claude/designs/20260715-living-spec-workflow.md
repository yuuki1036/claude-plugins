...
- **eval 回帰（`evals/runner.py`）は実施済み: 12/15（pass^k=3）**。ケース定義は `evals/cases/living-spec-workflow.yaml`（15 ケース。design-doc / adr-keeper との弁別を測る誤発火防止 4 ケースを含む）
  ...
  - **残る 2 件の fail は harness 側の性質**で living-spec の欠陥ではない。手を触れていない `notebooklm-workflow` でも同じ「実在しない skill id の捏造」が再現する（`summarize-ja` が `notebook-summarizer` を綴って落ちる）。**expected を緩めて通さない**（実在しない id を正解扱いすることになるため）
  - **id 捏造は確率的**で、同じケースが k=3 では通ることがある（notebooklm は k=1 で 3 件落ちたが k=3 では 1 件に減った）。**k=1 の結果を「恒常的に落ちる」の根拠にしない**
