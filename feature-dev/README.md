...
1. 候補となる曖昧点を列挙
2. 事実（Phase 2 explorer 結果 / Grep）と、明示的な決定記録（BDD spec / Issue の決定 / ADR）が答えている決定は自己解決し、質問から落とす
...
候補には常に前提確認の 4 分岐（設計システムとの対応 / 引き継いだ決定の前提が今も成り立つか / 必読 doc との衝突 / ユーザーの説明とコードの食い違い）を含める。前提の崩れや衝突は自己解決せずユーザーに聞く。自分で解決してよいのは事実と、明示的な決定記録が直接答えている決定だけで、コードの前例は推奨の根拠にとどめる。
