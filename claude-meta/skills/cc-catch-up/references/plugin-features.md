...
| `PreCompact` | v2.1.105 | コンパクション前 | コンパクションのブロック/前処理。exit code 2 または `{"decision":"block"}` でブロック | 重要コンテキスト保護が必要なプラグイン |
...
| `effort` | v2.1.80 | エフォートレベル（`low`/`medium`/`high`/`xhigh`/`max`）。`xhigh` は v2.1.111 追加（Opus 4.7 以降で使用可） | タスク複雑度に応じた設定 |
...
| `effort` | v2.1.80 | エフォートレベル（`low`/`medium`/`high`/`xhigh`/`max`）。`xhigh` は v2.1.111 追加（Opus 4.7 以降で使用可） | タスク複雑度設定 |
...
| 制約 | Since | 説明 | 適用シグナル |
|------|-------|------|------------|
| description 1,536 文字上限 | v2.1.105 | description の listing cap が 250→1,536 文字に拡大。超過時は起動時警告 | より詳細なトリガーフレーズ・使い分けガイドを記載可能 |
...
| `effort` | v2.1.84 | モデル effort レベルのオーバーライド（`xhigh` は v2.1.111 から Opus 4.7 以降で使用可能） | コマンド呼び出し時の思考深度制御 |
...
| `monitors` | v2.1.105 | バックグラウンドモニター定義。セッション開始 or スキル invoke 時に自動起動 | 継続的なバックグラウンド監視が必要なプラグイン |
...
| 機能 | Since | 説明 | 適用シグナル |
|------|-------|------|------------|
| `/reload-plugins` | v2.1.69 | セッション中プラグイン再読み込み（v2.1.98 でスキル追加も即時反映） | 開発時ホットリロード |
