...
2. **コードベースの確認**:
   ...
   - 依頼の言葉だけでなく、同じ概念の別名でも探す（「認証」なら `auth` / `session` / `login` / `credential`）
3. **判定と提示**:
   - 確認結果が空 or 関連薄: 探した場所と語（Glob のパターン・Grep の語）を 1 行で報告して Phase 5.5 へ進む（「無かった」を検証できる形で残す）
...
**1. knowledge**
1. `{DATA_DIR}/{slug}/knowledge/index.md` があれば Read し、tags 列とキーワードを照合する
2. index.md が無ければ `{DATA_DIR}/{slug}/knowledge/*.md` を Glob し、各フロントマター（tags）と照合する
3. 併せてキーワードで `{DATA_DIR}/{slug}/knowledge/` を grep し、tags に載らない本文一致も拾う

**2. 完了 Issue**
- `{DATA_DIR}/{slug}/issues/` をキーワードで grep し、ヒットしたファイルのうち frontmatter が `status: completed`（または `canceled`）のものを「過去に解いた/判断した類似 Issue」として抽出する（未完了 Issue との重複起票チェックとは目的が別なので、ここでは過去に解決・判断済み＝completed / canceled に絞る）
...
- Issue の内容が提示した ADR とぶつかるなら、「備考」に ADR の id・ぶつかる点・見直す理由を 1 行で書く（黙って上書きしない。見直すなら ADR の supersede が別途要る）
- 1 件も見つからなければ何も追記せず次 Phase へ進む（ノイズを出さない）

### Phase 6: Issue ファイル生成

1. **配置先**
   - `{DATA_DIR}/{slug}/issues/{ISSUE-ID}.md`
