...
無効な場合は「このスキルは Linear 連携プロジェクト専用です（backend=linear）。ローカル backend では `/issue-workflow:start`（main ブランチのダッシュボードモード）を使ってください。」と案内して終了する（dormant 検出と同型のガード）。

## Phase 0.5: Linear MCP 利用可能性チェック（両モード共通）
