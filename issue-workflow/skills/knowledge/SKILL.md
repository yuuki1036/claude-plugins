...
name: knowledge
description: >
  プロジェクトの蓄積された知見を検索・参照する（一覧 / キーワード検索 / 関連表示。読み取り専用）。
...
- 解決対象: `{DATA_DIR}/{slug}/knowledge/` 配下（`concepts/` を含む）の `.md` ファイルの basename
...
3. **index.md がない場合:**
   - `{DATA_DIR}/{slug}/knowledge/**/*.md` を Glob で列挙（`concepts/` 配下も含む）
...
1. 現在の Issue ファイルを特定する:
   ...
   - `{DATA_DIR}/{slug}/issues/{ISSUE-ID}.md` を Read する
