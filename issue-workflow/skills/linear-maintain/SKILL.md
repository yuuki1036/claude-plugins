...
このスキルは**起動した時点で実行確定**とみなす。実行可否やスキャンモードを起動時に問い直さない（AskUserQuestion で止まらない／ストレスフリー設計）。スキャン範囲は常に全対象（処理 1〜6。slug 指定時はそのチームのみ）とし、**走査深度のみ**実行時 effort = `${CLAUDE_EFFORT}` で調整する（下表）。結果は最終レポートにまとめて報告する。

### effort 適応（走査深度）

| effort | 走査深度 |
|--------|---------|
| low / medium | 同期・検出系（処理 1〜5 と 6a の status 更新）のみ実施。6a のメンテナンス処理（圧縮・knowledge 切り出し）と 6b の品質整理は省略し、対象件数だけレポートに残す（速度優先） |
| high | 全処理（1〜6。6a/6b で全 Issue に issue-maintain の全処理フローを適用）を実施 |
| xhigh / max | 全処理に加え、knowledge 切り出し候補の重複排除を全チーム横断で網羅し、6b の整理対象走査（削除/圧縮/統合）を全セクション精読で行う |

effort によらず「止まらず実行し切り、実行後レポートで報告する」方針は不変。省略した処理はレポートに `省略（effort: {値}）` と明記する。

## Phase 0: backend ガード（linear 専用スキル）

このスキルは Linear 連携 backend 専用。Glob で `.claude/linear/*/` を確認し、「dir が存在し、かつプロジェクト slug サブディレクトリを 1 つ以上持つ」場合のみ続行する。
無効な場合は「このスキルは Linear 連携プロジェクト専用です（backend=linear）。ローカル backend の棚卸しは `/issue-workflow:maintain` を使ってください。」と案内して終了する（dormant 検出と同型のガード）。

## Phase 0.5: Linear MCP 利用可能性チェック
