...
- `CR=1`: `Skill` tool で `code-review:self-review` を起動する（引数は現在の base に合わせる）。
  指摘への対応は self-review 側の修正方針フローに従う。**self-review の Step 6 レポートは定型のまま出す** —
  下の「報告」の Step ごとの 1 行はその後に足すもので、定型の代わりにしない（1 行に畳んだ回は
  指摘件数も閾値未満の候補もユーザーに届かなかった）。self-review の手順はここに複製しない
