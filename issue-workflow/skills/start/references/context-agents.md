...
値がある場合:
- `{DATA_DIR}/{slug}/issues/{PARENT-ID}.md` を Read する
...
各 Issue ID について:
- `{DATA_DIR}/{slug}/issues/{RELATED-ID}.md` の存在を確認（Glob）
...
プロジェクト doc（local: project.md / linear: projects/*.md）の内容を確認し、Issue のタイトル・概要に関連するセクションを特定する。
...
## Agent #3: Linear Sync（Linear 同期チェック・BACKEND=linear のみ）

Linear API から最新情報を取得し、ローカル Issue ファイルとの差分を検出する。起動には `linear-sync` agent 定義（`agents/linear-sync.md`）を使う。
