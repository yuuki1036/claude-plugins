...
- **軽量フローの説明文が正本と矛盾していたのを直した**（v1.6.0 の self-review 指摘）。start SKILL.md の F7
  で「self-review / commit の委譲先も導入済みのときだけ使う」と書いていたが、正本の
  `lightweight-flow.md` は未導入時の手動フォールバックを持ち（CHANGELOG も「未導入なら手動」と明記）、
  「導入済みでないと軽量フローを使えない」と誤読させる齟齬だった。「導入済みなら委譲・未導入なら手動、
  選択肢自体は委譲先の有無に関わらず提示」に修正。
...
- **同名 command の本文に SKILL.md への Read 誘導を置いた**（GitHub issue #219）。command 名と skill 名が
  ...
  本文に書いた。`validate_plugin_quality.py` の `skill-hop-cmd` が error で強制する。対象: `dashboard` / `follow-up` / `init` / `issue-create` / `issue-design` / `issue-maintain` / `knowledge` / `knowledge-lint` / `linear-maintain` / `maintain` / `retrospective` / `start`
...
- **同名 command の description に `トリガー:` を複製した**（GitHub issue #206）。command 名と skill 名が
  ...
  ルーティングには効いていなかった。対象: `dashboard` / `discover` / `follow-up` / `init` / `issue-create` / `issue-design` / `issue-maintain` / `knowledge` / `knowledge-lint` / `linear-maintain` / `maintain` / `retrospective` / `start`。
...
- **Linear Issue の内容を判断材料にする経路でコメントも取るようにした**（GitHub issue #201）。
  `issue-create` の BACKEND=linear 経路は `get_issue` だけを呼び、description を材料に
  Issue ファイルを生成したうえヒアリングまでスキップしていた。Linear の description は
  起票時のスナップショットで、その後の仕様変更・スコープ削減・保留理由はコメント側に
  積まれるため、本文だけを見ると誤った前提のまま Issue が固定される（本リポジトリの
  CLAUDE.md 「記録から現在の状態を断定しない」と同型）。`list_comments` をセットで呼び、
  本文と食い違う記述は後発のコメント側を採って採用理由を 1 行提示する
- **同期チェックのコメント取得上限を 5 件から 50 件に上げた**（同 issue の 3 点目）。
  `linear-sync` agent は「最終更新日以降の新着コメント」を出すのに最新 5 件しか読んでおらず、
  最終更新から間が空いた Issue で新着を取りこぼしていた（決定がコメント欄の上流に沈む）。
  あわせて、返却が上限に達した回は「読めていない範囲がある」と明示する規約を足した
