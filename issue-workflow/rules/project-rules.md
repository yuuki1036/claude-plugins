...
- 調査・実装・テストなど**独立した作業**は並列エージェントに分割してよい（起動に確認は不要）
- ただし**同時起動は 1 タスクあたり 4 体まで**。超える場合は段に分ける（委譲はコンテキスト再構築のコストを伴う。体数上限は暴走ガード）
- 数回のツール呼び出しで済む小さな作業は委譲せず直接実行する（委譲のオーバーヘッドが本体を上回る）
