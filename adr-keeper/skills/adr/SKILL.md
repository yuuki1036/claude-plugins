...
**記録価値の 3 条件セルフチェック**（手順に入る前に自問する。ゲートの目的は ADR の希釈防止 — 些末な記録が混ざるほど、重要な ADR が読まれなくなる。**Phase 4 supersede からの呼び出しと、design-doc Phase 6 からの切り出しでは適用しない** — 前者は覆される決定が既に ADR として存在すること自体が 3 条件の充足を示し、後者は design doc の `[→ADR候補]` からユーザーが選んだ時点で選別が済んでいるため）:
