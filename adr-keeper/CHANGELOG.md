...
- **adr-write-guard のパス正規化の 2 行に `# mutation-ok` を付けた**（挙動は変わらない / #244）。条件を反転すると
  `/./` や `//` を含まないパスで置換が空回りして無限ループになり、変異テストでは生存ではなく TIMEOUT に分類されていた。
  あわせて「frontmatter は 1 行目の `---` からだけ読む」の回帰テストを足した（本文の `id:` 行と区切り線を
  frontmatter と取り違える変異が生存していた）
...
- **本文の必須節が README と SKILL.md で食い違っていた**。README は 8 項目すべて必須としていたが、SKILL.md の完了報告は
  「検討した代替案」「関連」を「必要に応じて補完」としていた。SKILL.md を README に合わせ、new の手順 7 で template.md の
  本文の節をすべて会話文脈から埋めるようにした。代替案は 3 条件の 3 で確かめたもの（supersede では旧 ADR の決定も含める。
  3 が NO のまま記録したならその旨）。関連は 4 項目のラベル行を残し、確かめて無い項目を「なし」とする。
  埋められない節は推測で埋めず、完了報告に未記入として挙げる（テンプレのコメントが 1 つでも残る節は未記入）。
  command 本文と README の表も合わせた
...
- **同名 command の本文に SKILL.md への Read 誘導を置いた**（GitHub issue #219）。command 名と skill 名が
  ...
  本文に書いた。`validate_plugin_quality.py` の `skill-hop-cmd` が error で強制する。対象: `adr`
