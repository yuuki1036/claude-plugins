...
2. 一定期間後に `/retro` で candidates を**承認レビューして journal に昇格**し、閾値超え（直近 30 日 × 3 回以上）のパターンを抽出する。閾値の分子は**最後の還流日以降の発生**で、既に手を打った発生は数えない（分母と除外件数は併記する）
...
### 還流の実施記録（remediations.jsonl）

閾値超え tag に対して**実際に打った手**を `.claude/failure-journal/remediations.jsonl` に append-only で記録します。`/retro` の閾値判定はこの記録より後の発生だけを分子に取ります。

```json
{"timestamp":"2026-08-28T00:00:00Z","tag":"spec-skipped-without-rationale","target":"convention","ref":"ac8214d"}
```

記録が無ければ「還流ゼロ」として従来どおり集計されるので、導入は任意です。

- **実施していない還流を書かない**: この記録より前の発生は分子から外れるため、提案しただけ・着手しただけで書くと再発を見逃します。書くのは還流が landed した後です
- **「還流後 0 件」と「鳴らない」は別物**: 還流していない tag の 0 件は無情報ですが、還流後の 0 件は対策が効いている可能性の観測なので、レポートでは別の節に出します
