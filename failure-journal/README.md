...
`/retro` は集計の前に **transcript を走査して未起票の失敗をサルベージ**します（Phase 0.5）。手動起票は「人間が失敗に気づいて想起したもの」しか拾えず、実測で起票率は約 2.5% でした（失敗の大半は Claude が自己訂正するためユーザーの目に触れない）。サルベージは grep 検知 → LLM 分類 → **承認制 append** で取りこぼしを回収します。
