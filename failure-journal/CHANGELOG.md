...
- **未レビューの候補がたまったら、SessionStart で `/retro` を提案させるようにした**。自己申告は書かれ続けるのに retro を
  思い出す契機が無く、実測で 08-30 以降の 39 件（うち 9 月分 31 件）が未レビューのまま溜まっていた（レビュー済みの
  39 件は全件採用で、却下は 0）。未レビューが 15 件以上か、最古の未レビューが 14 日以上前なら、ルールの末尾に 1 行足す。
  PostCompact では出さない。数えるのは verdict と ts だけで、summary は読まない

### Changed

- **還流先の判定に 2 つ足した**（aggregation-rules.md）: 失敗を捕まえられる検査が既にあるのに配線されていない・黙って
  壊れているなら、新しい検査を作らず配線・修理を提案する。情報に辿り着けなかった失敗には、規則ではなくナビゲーション用の
  ポインタを提案する（翻案元: mattpocock/skills の retro）
