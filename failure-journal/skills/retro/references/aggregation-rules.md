...
| 情報に辿り着けなかった（正本の場所・隠れた依存・読むべき doc を知らなかった） | **ナビゲーション用のポインタ**（参照元の SKILL.md / references / CLAUDE.md に「X をするときは Y を読む」を 1 行） | 規則を足すより安い。規則は守らせるもの、ポインタは見つけさせるもの |

判定の補助質問:

- **その失敗を捕まえられる検査が既にあるのに、配線されていない・黙って壊れていないか？** → 先に確かめる（pre-commit・CI・`validate_*.py`・hook・`package.json` の lint / test）。既存の検査が効いていないならそれが所見で、新しい検査は作らず配線・修理を提案する
...
- 検証はあるが配線されていない・黙って壊れている（呼ばれていない・exit code を見ていない・例外で素通り） → 配線・修理を提案（新規作成より先）
- 情報に辿り着けなかった → ポインタを提案
