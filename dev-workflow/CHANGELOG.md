...
- **diagnose で翻案時に落ちていた部分を補った**（翻案元 mattpocock/skills diagnosing-bugs の `ee8bae4`）。loop が作れない
  ときにユーザーへ頼むキャプチャに core dump を、計装の許可に本番（または再現する環境）への計装を足した。正解だった
  仮説は PR 本文にも残す（伏せ字の対象にも PR 本文を足した）。Phase 0 の「使う」の性能劣化を「以前より遅くなったという
  症状がある」ものと肯定形で書いた
- **Phase 4 の完了時に、確証した仮説・原因の `file:line`・修正方針を提示してから修正に入るようにした**。Phase 3 と同じく
  返答を待って止まらない（止めて承認を待つのは起動時に頼まれた場合だけ）。翻案元にも無い確認の機会で、止めない形にした
...
- **pr-creator が repo の PR 分類・リスクラベル手順を実行しないまま PR を作ることがあった**（GitHub issue #234）。
  Step 1.4 は repo skill の手順を「desc 生成 / review / その他」に分けるが、分類がどれに入るか書いておらず、
  「review」側に落ちるとラベルも判定根拠も付かなかった。
  - PR の分類 → リスクラベル → 判定根拠の記入を 1 組の「その他」手順と明記した。分類はラベルを決める前段で review ではない
  - 実行時期を決めた: 分類と根拠は PR 番号が要らなければ Step 4 の body に含めて承認を通す。作成済み PR が前提なら作成後に実行し、根拠を追記した body は再承認してから更新する
  - Step 4.93 のチェックリスト 1 行目に、取り込んだ手順と実行時期を書かせる
  - Step 5 の作成後レポートに、取り込んだ手順ごとの `実行済み` / `未実行: <理由>` を出す
...
- **ui-verify の verify モードで全ケースの verdict を必須化**（GitHub issue #228）。正常系だけ通して完了しても
  検知されなかったため、次の 3 点を構造化した:
  - **coverage 宣言**: ケース列挙の時点で種別（正常 / 準正常 / 異常）ごとの件数と固定 5 項目の該当・非該当を
    表で出す。準正常 / 異常が 0 件なら件数ではなく**落とした理由**を書く（「該当なし」では通さない）
  - **verdict の空欄禁止**: 列挙した全ケースに pass / fail / blocked を付けるまで完了にしない。実行しなかった
    ケースは空欄ではなく `blocked` + 理由で残す（正常系だけで打ち切った事実が表に出る）
  - **未カバーの明記**: 準正常 / 異常が 1 件も pass していなければ `未カバー` をサマリと verification.md の
    両方に書く。verification.md に `## カバレッジ` セクション（種別 × pass/fail/blocked の集計 + 固定 5 項目 +
    未カバー行）を追加し、pr-creator がこの行を PR 本文へ転記する
- **pr-creator に必須サブステップの pre-flight ゲート（Step 4.93）を追加**（GitHub issue #226）。必須と明記した
  手順が実行側の判断で黙って任意に格下げされる穴を塞ぐ。Step 4.95 の提示前に 7 行のチェックリストを出力し、
  各行を `done` / `N-A: 理由` で埋めるまで提示へ進めない fail-closed ゲート:
  - 対象は Step 1.4（repo 固有 PR skill の取り込み）/ 4.3（writing-polish）/ 動作確認の大セクション生成 /
    未カバー行の転記 / 4.5（Screenshots）/ 4.7（三要素セルフチェック）/ 4.9（body の機械検証）
  - 4.7・4.9 は `N-A` 不可。writing-polish は「インストール済みかつ未適用」を `N-A` にできない
  - body に及ぶ修正で 4.3 → 4.7 → 4.9 をやり直した場合はゲートも再出力する
  - 動作確認は `## 動作確認` の大セクションとして生成する（チェックリスト 1 行へ潰さない）ことを本文と厳守ルールに明記
