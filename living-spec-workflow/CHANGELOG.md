...
- **仕様表を凍結できるようにした**（format-spec 2.1 節 / #245）。仕様の正本を別の文書へ移したあとも living spec を
  Decision log と OQ 台帳として使い続けると、触らなくなった `方向性(仮)` / `未定` の行を maintain 段 6 が毎回
  塩漬けとして出し続け、本当に塩漬けになっている項目が埋もれていた（実例: 72 項目中 32 項目・最長 86 日）
  - `/living-spec spec --freeze D<n>` で凍結、`--unfreeze D<m>` で解除する。どちらも先に Decision で経緯を残し、
    その D# を渡す。frontmatter に `spec_table: frozen` と `spec_table_frozen_by: D<n>` の 2 行を書く / 消す。
    凍結の Decision は表の最後の更新以降のもの、解除の Decision は凍結の Decision より後のものに限る
  - 凍結中は、`spec` が追加・更新を拒否し、`status` が収束率を数えず「凍結済み」と出す（表の内訳は参考）。
    maintain 段 6 は塩漬けを判定せず、そのことを「skip した段」に理由つきで出す。代わりに、凍結の Decision より
    新しい `since` の行（凍結後の手編集）を Warning にする
  - `decision` は、採番した番号を凍結の宣言が既に指していたら（まだ無い Decision を指す手編集の誤記）、書き込まずに止める。
    そのまま書くと、その Decision が凍結の根拠として読まれる
  - 凍結の 2 行の書式違反は段 1、凍結の Decision が無いのは段 3 の Critical。どちらのときも宣言を無効として扱い、
    段 6 は塩漬けを判定する（書き損じ 1 つで段 6 が黙らないように）。壊れた宣言は `--unfreeze` を D# なしで実行すると
    消せる（書き損じの修正なので Decision を要さない。Decision を書き足して番号を合わせると、無関係な Decision が
    凍結の根拠として読まれる）
  - 表全体のみで、行単位では凍結しない。確度ラベルに 4 つ目の値を足す案は、3 値の契約と段 1 の正規表現を
    変えることになり、凍結した時点の確度も失われるので採らなかった
...
- README のセクション表の「現在地サマリ（3 行以内）」を、0.4.0 のテンプレ（1 行目に到達点、続けて 3 行以内）に揃えた

## [0.4.0] - 2026-09-24

### Added

- **到達点・まだ詰められないこと・スコープ外を散文で書く場所を作った**（mattpocock/skills の wayfinder との比較から、
  表スキーマを変えずに取れる部分だけを採った）。テンプレの「現在地サマリ」は 1 行目に到達点（何を決め終えたら
  閉じるか）を書く。「進め方フェーズ」には「まだ詰められないこと」「スコープ外」を H3 の小見出しで置ける。
  スコープ外と分かった OQ は decision で閉じ、見出しを `スコープ外: 〜` にする。パース対象の 3 セクションには触れていない
- 「いつ使う」に使い分けの軸（1 セッションで片付くなら design-doc / feature-dev の grill、複数セッションに跨るなら
  living spec）と「決める場所で、実装はしない」を足した。init の確認でも、1 セッションで片付きそうなら 1 文だけ添える
- decision の完了報告で、close した OQ に問いの先頭を添える（番号だけでは読めない）
- status の最後の案内を渡し先 3 つ（HOW → design-doc / WHAT → bdd-spec / 作業 → issue-create）にした
- living-spec-maintain の段 8 に「到達点の外にある OQ・項目が増えている」の例を足した
