...
# **打点ファイルの識別子は session id**（GitHub issue #247）。Bash 側（review-timing.sh）は env の
# `CLAUDE_CODE_SESSION_ID` から作るので、hook も同じ id から作らないと別ファイルを見る。hook の
# env に載ることは保証されていないので、stdin の `session_id`（全 hook 共通のフィールド）を優先する
SESSION_ID=""
if command -v jq &>/dev/null; then
  SESSION_ID=$(printf '%s' "$INPUT" | jq -r '.session_id // empty' 2>/dev/null || true)
else
  SESSION_ID=$(printf '%s' "$INPUT" | grep -oE '"session_id"[[:space:]]*:[[:space:]]*"[^"]+"' | head -1 | sed -E 's/.*:[[:space:]]*"([^"]+)"/\1/' || true)
fi
...
MSG="code-review の計測が start されたまま publish（review:completed）されていない:${pending}。レビューが継続中なら無視してよい（この通知は打点ファイルごとに 1 回だけ）。publish せずに中止したレビューなら publish せず \`bash \"\${CLAUDE_PLUGIN_ROOT}/scripts/review-timing.sh\" discard\`（review は \`--pr N\` 付き）で打点ファイルを捨てる。レビューが終わっているなら publish を踏む（self-review Step 6.4 / review 締めフロー 4 — \`bash \"\${CLAUDE_PLUGIN_ROOT}/scripts/publish-review-event.sh\"\`）。レポート出力から時間が経つほど duration が伸び、self-review は 10 分以上で欠測に倒れる。SKILL.md を読まずに記憶から手順を再現した回は publish が丸ごと落ちる（GitHub issue #219）。"
echo "WARN: ${MSG}" >&2
