...
このレイヤーの目的は「指摘を増やす」ことでも「指摘を一律に潰す」ことでもなく、**reviewer 既存の自己検算（共通指示「退行指摘の invariant 検算」「事実主張のツール接地」「severity を付ける前に: base 状態の確認」）では届かない、独立読み直しでしか分からない過大評価と偽陽性を摘出**すること。meta-reviewer（見落とし＝false negative を足す係）の鏡像にあたる。

**実測上、返る verdict で最も多いのは `severity_inflated` で、`refuted` は少数**（比率の正本: `design-notes/scoring-rationale.md`）。**「潰す」より「較正する」方が主機能**だと理解して臨むこと — 取り下げに値しない指摘を無理に `refuted` に寄せない。
