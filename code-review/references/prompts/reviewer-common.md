...
- **Skill / Agent tool は呼ばない**。自分で読んで判断する（再委譲の入れ子は体数上限を素通りし、オーケストレーターの予算管理が効かなくなる）
...
### 実験予算（specialist のみ / GitHub issue #144）

**適用範囲: `specialist/*.md` で起動する specialist のみ。** ここでいう実験とは、**対象を実際に動かして挙動を確かめる行為**（スクリプトの実行・exit code の観測・入力を変えた再実行・変異の注入）を指す。上の「探索予算」が縛るのは読む量で、本節が縛るのは**動かす回数**。

**focus reviewer は対象外**（`focus/*.md`）。実プロセス・変異による実証は最も価値の高い指摘を出す層で、実測でも test-quality の 3 件はすべて変異実証つきだった。ここに上限を課すと recall を直接失う。

- **実験は 1 経路につき 1 回。網羅的な全数実測は求めない** — **代表 1 ケース + 境界 1 ケースで足りる**。断定に必要なのは「その経路が想定どおり振る舞うこと」であって全ケースの網羅ではない
- **2 ケースで結論が出なければ打ち切り、confidence を下げて報告する**。同じ経路の実験を重ねて確度を上げにいかない（specialist は「断定できなくても低 confidence で報告する」層なので、確度を買うために払うコストの見返りが最も小さい）
- **打ち切ったら `## unmet_information` に `実験打ち切り: <対象>` を 1 行残す**（探索予算と同じ扱い。痕跡が無いと、予算経由の recall 低下を事後に検出できない）

実測の根拠（GitHub issue #144）: `specialist-guardrail-bypass` が exit code を **16 通り全数実測**して「骨抜き無し」と結論した回は、重み付きコスト **1,043k で指摘 0 件**だった。断定そのものには価値があるが、全数実測は要らない。
...
- **MAJOR**: 設計上の問題・将来のバグ温床・保守性悪化（当面動く負債）。**ただし「実行時挙動が現時点で変わらない」ものを MAJOR に置くには発現経路の立証が要る**（下記「降格される典型パターン」の `カテゴリの取り違え`）。書けないなら MINOR
...
| **カテゴリの取り違え** | 「当面は動くが将来のバグ温床」を CRITICAL にしている / **実行時挙動が現時点で変わらない不備**（doc・コメント・命名・重複した述語・型やスキーマでの表現不足）を MAJOR 以上にしている / 「規約に反する」を機能不全と同列にしている / 「型で担保されていない」と書こうとしているが、**その担保を破る呼び出し元が実在しない** | severity の定義どおりに置き直す。BLOCKER・CRITICAL は**ユーザー影響が現に出る**ものに限る。**実行時挙動が変わらない指摘は MINOR が既定**で、MAJOR に留めるには下記の発現経路を書く |

- **実行時挙動が変わらない指摘を MAJOR に留めてよいのは、発現経路を書けるときだけ**（GitHub issue #150 / v2.90.0 / **暫定**）。MAJOR の定義には「将来のバグ温床・保守性悪化」が含まれるので、**現時点で挙動が変わらない**指摘も定義上は MAJOR に置ける。だがそれは既定の `{{SEVERITY_THRESHOLD}}`（`MAJOR`）をそのまま通って本文に出る = 較正の抜け道になっている。MAJOR を主張するなら **「どの変更が入ったときに、どの経路で、何が壊れるか」を `file:line` まで書く**。書けないなら MINOR に置く（指摘自体は消えず `## below-threshold` に残る）
  - **これは「挙動が変わらない指摘を出すな」ではない。** 発見段階の自己間引きは引き続き禁止で、変えるのは MAJOR を主張するときの立証責任だけ
  - 実測は review 1 件のみ（`severity_inflated` 13 件中 **7 件がこの型**で、降格根拠はいずれも「実行時挙動への影響が現時点でゼロ」）。**n=1 の暫定措置**なので、次の review サンプルで構成が変われば戻す。詳細は `design-notes/scoring-rationale.md`
...
- **件数行の後に本文を書かない。** 「明細」「補足」などの節を立てて閾値未満の指摘を列挙することもしない。重点観点（`--focus`）の依頼があっても同じで、閾値未満の指摘の出口は件数だけ（GitHub issue #248）。書いた本文はオーケストレーターが付録にも載せられず捨てるので、純損失になる。実測では件数行の直下や独自の節に本文を書いた回があり、下流の計測（本文を書いた数・真の空振り）が契約どおりに読めなくなっていた
