...
#### 確定事実（全 reviewer 共通 / 最大 5 項目・各 1 行）
- [ファイル:行] 観点を問わず複数の reviewer が同じ確認をしそうな、**この diff で確定した事実**（例: 「`next.config.ts` に `serverActions` の設定が無く、Next 既定の 1MB 制限が効く」「この関数の呼び出し元は 3 箇所で、いずれも戻り値を捨てている」）
```

**`#### 確定事実` 欄の書き方（GitHub issue #122）**: この欄だけは**全 reviewer に共通で注入される**（他の欄は依存する reviewer にのみ渡る）。同じファイルを 5 体が独立に読み直す重複を消すための欄なので、以下を守ること:

- **自分で Read / Grep して確認した事実だけ**を書く（推測・伝聞・「〜のはず」は書かない）。**判定は書かない**（「危険」「バグ」は他の欄と同様に禁止）
- **裏取りの根拠 `ファイル:行` を必ず添える**（reviewer はこれを再確認せずに指摘の根拠として引用する）
- **観点横断で効くものに絞る**。1 観点にしか効かない事実は `#### 重要な発見` に書く。**該当なしなら欄ごと省略してよい**（無理に埋めない）
- 5 項目・各 1 行を超えない（全 reviewer のプロンプトに複製されるため、長さがそのまま体数倍のコストになる）
