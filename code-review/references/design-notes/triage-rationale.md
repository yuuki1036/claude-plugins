...
### 相関の再監視を終了した根拠（GitHub issue #217 / v2.116.0）

`## 7` の「体数を壁時計のレバーとして扱わない」は #116 で **review 13 件・medium 帯のみ**の「無相関」を根拠に置いていた。サンプルが 213 件に伸びると、その事実主張は tier によって成り立たない:

| 層 | n | r |
|---|---:|---:|
| medium/unrecorded | 45 | 0.156 |
| small/unrecorded | 16 | 0.591 |
| large/unrecorded | 12 | **0.665** |
| small/opus-4-8 | 12 | **0.687** |
| medium/opus-4-8 | 10 | 0.466 |

**⚠️ シグナル（`|r|` が 0.6 以上）は 3 回鳴り、3 回とも調べ切った**:

1. **tier 交絡**（#151）— 層別で medium は 0.32 まで落ちたが small / large は残った
2. **世代・effort の交絡 / wave 数の間接経路**（#217）— `xhigh/small/opus-4-8` で 0.673。wave 数を統制した偏相関でも small 0.866 / medium 0.697 とほとんど落ちない
3. **synthesis（agent 非稼働区間）の混入**（#217）— `fleet − synthesis` で取り直しても small 0.830 → 0.827 でほぼ動かない

**どれでも消えない＝この指標から次に調べる先が無い。** 鳴らし続けると「⚠️ が出たときだけ行動する」契約の方が壊れるので、**シグナルを撤去して表と 1 行の注記に降格した**（撤去しても計測は続く。表は世代でも層別するようにした — 相関だけ `size_tier` 単独で、tier 中央値表は `with_gen` という粒度の食い違いがあった）。

**採らなかった案**（#217 の候補①②）:
- **層ごとに水準を変える**: データに合わせて閾値を動かすのは検出力を捨てるのと同じで、「初回実行で偽陽性が出る warning は入れない方がまし」の裏返しの誤り
- **偏相関で判定する**: 上の 2 のとおり統制しても 0.7〜0.87 で残るので、判定が変わらないうえ retro に無い計算が増える

**再開の条件は指標ではなく実験**: #190 の A/B が「体数削減で fleet が縮み、かつ MAJOR が減らない」を示したら `## 7` ごと見直す。相関の再点灯では再開しない。
