...
## `## 3` 共通モジュール explorer 必須ルールに規模下限を入れた根拠（v2.61.0 / GitHub issue #122）

v2.12.0 は「共通モジュールの変更は**行数や関数数に関わらず**必ず explorer 1 体」と定めた（小規模変更でも依存元への波及を見落とさないため）。意図は正しいが、**波及先が数えられて少ないケースにも一律で 1 体が張り付く**。

**実測**: `app/lib/withActionErrorHandler.ts` の **型引数を 1 つ足しただけ（`+5 -2` 行）** の変更に explorer 1 体が起動し、**13.6 万 tokens・35 tool_uses** を費やして「問題なし・後方互換」と結論した（結論自体は正しく、実測での裏取りも丁寧だった）。

**下限を「呼び出し元の件数」に置いた理由**: 見落としリスクは変更の行数ではなく**波及先の数と、呼び出し側の書き換えを強制するか**で決まる。行数で切ると「5 行だが破壊的な signature 変更」を取りこぼす。逆に呼び出し元が数件なら、reviewer 自身の Read で全件当たれる（explorer の存在意義＝「reviewer が読み切れない広さ」が消える）。

- `importers` は `triage-signals.sh` が **basename の word 一致**で数える**過大見積もり**（import 以外のヒットも拾う）。過大側に倒しておけば、下限判定は「起動する」側に転ぶので安全側
- **数えられない場合（`?`・basename が 3 文字未満）は起動する**。欠測を「少ない」と読まない
- **撤回条件**: 下限で explorer を見送った回に、共通モジュール起因の指摘を後から人間が見つけた事例が出たら、下限（5 件）を下げるか条件 2（破壊的変更でない）を厳しくする。**下限そのものを撤去する前に閾値を動かす**
...
**meta-reviewer を昇格しなかった理由（v2.52.0 当時）**: meta は reviewer 全結果に依存するため相乗りできず、**直列 wave を 1 本足す**。壁時計が wave 数に支配される以上（`## 5.1`）、skeptic と同じ扱いにはできなかった。

> **この理由は v2.61.0 で失効した**（`orchestration-rationale.md`「meta-reviewer と反証レイヤーを同一 wave にした経緯」）。meta は反証レイヤーと同一 wave になり、反証が走る帯（effort ≥ high）では**追加の直列 wave が 0 本**になる。**昇格の可否は wave コストではなく `meta_reviewer.findings_added` の価値率で判断する**（skeptic と同じ流儀。サンプルは `gate_schema >= 2` で絞る）。「相乗りできないから無理」を据え置き理由に使わないこと。
...
### 相関シグナルは `size_tier` 内で取る（GitHub issue #151）

上の判断（体数は壁時計のレバーではない）は `size_tier` を揃えて出したものだが、**`review-retro.sh` の ⚠️ シグナル側は層別せず全サンプルで相関を取っていた**。`size_tier` は体数（`triage-guide.md ## 7` の体数表）と fleet 時間の**両方**を決めるので、層別しない相関は tier の効果を体数の効果として計上する。全 9 リポジトリ n=83（相関の母集団 n=48）の実測:
...
**最大サンプルの medium で r=0.315 に落ちる**。`7 体 → 126 分` と `12 体 → 61 分` が同じ tier に共存しており、体数以外の要因が支配している。旧シグナルは全 PC 集計・単一リポジトリ集計の両方で常時点灯しており、CLAUDE.md の「初回実行で偽陽性が出る warning は入れない方がまし」に該当していた。

**打ち手**: 相関を tier 内で計算し、**発火条件は層別後の r だけ**で判定する。層別なしの r は「tier 交絡を含む参考値」として表示だけする。tier ごとの下限は層別なしと同じ `R_MIN_N`（緩めると単発の tier が点灯する側に倒れる）。**`## 7` の再監視条件（`|r| >= 0.6`）の水準そのものは変えていない** — 層別後の値が各 tier で貯まってから判断する。

**併走して観測する仮説**（サンプル 2 件では判定しない）: `dispatch.schema=2` の 2 件が `waves=4` / `5` で `max_inter_wave_sec` が 1191 / 2043（＝ wave 間の待ちが 20-34 分）。「体数 → wave 段数 → 壁時計」の間接経路が疑われる（#142 / #149 の計測待ち）。
