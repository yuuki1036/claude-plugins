...
**再挑戦するなら**: 区別するための**表記の規約**が先に要る（例: 当該変更を指すラベルにだけ専用マーカーを付ける）。規約なしに検査だけ足すと、**偽陽性で「⚠️ が出たときだけ行動する」契約を壊す**ほうが損。「ラベルが現行版より新しい」（未来の版を書いた）だけなら偽陽性ゼロで検出できるが、**実際に踏んだ失敗（古いまま）は捕まらない**ので入れていない。
