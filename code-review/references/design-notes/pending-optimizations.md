...
## 11. reviewer 1 体あたりの往復上限（#156 Phase 2 / GitHub issue #190）— **判定基準を先に確定・A/B 待ち**

**まだ入れていない。** 入れるのは A/B で recall への影響を測ってからで、**予算だけ先に入れない**
...
`agents_abandoned` 0）。1 run 内なので tier / effort / 世代は定義上そろう。

| ロール | focus | 往復 | cache_read | /往復 |
|---|---|---:|---:|---:|
| reviewer | test-quality | 80 | 10,997k | 137.5k |
| reviewer | bug-detection | 74 | 8,690k | 117.4k |
| reviewer | claude-md-compliance | 47 | 6,129k | 130.4k |
| reviewer | comment-accuracy + polish | 42 | 4,511k | 107.4k |
| reviewer | cross-cutting | 37 | 3,934k | 106.3k |
| explorer | retro aggregation flow | 42 | 3,434k | 81.8k |
| explorer | publish tokens/dispatch | 23 | 1,487k | 64.6k |

全体 r = 0.978 / reviewer 内 r = 0.979。**ロール別の単価差も再現**（reviewer 106〜138k /
explorer 65〜82k）。#156 の値（reviewer 98〜124k / explorer 63〜74k）とほぼ同じで、
上限を置くならロール別にするという判断は 2 run で支持されている。

### この run が示した recall 側のシグナル（**A/B の代わりにはならない**）

往復を実測値で外挿した削減量と、打ち切りに掛かる体:

| cap | 削減 | 打ち切られる体 |
|---:|---:|---|
| 50 往復 | 18% | test-quality, bug-detection |
| 40 往復 | 28% | 上記 + claude-md-compliance, comment-accuracy, retro-aggregation |

**打ち切り対象になる 2 体が、この run で最も深い裏取りをした 2 体だった。**
`bug-detection`（74 往復）はこの run で唯一の MAJOR を出し、しかも「その欠陥が既存 4 箇所に
遮蔽されて現状は到達不能」までを合成ログの実行と `git blame` で裏取りしている。
`test-quality`（80 往復）は変異を実際に当ててテストが空虚 pass でないことを確認している。
どちらも探索予算が縛る当の「裏を取りにいく動き」。

**ただしこれは A/B ではない。** cap 到達前に同じ結論へ至れたかは分からない（往復の前半で
見つけて後半は裏取りに使った可能性がある）。**それを分離するのが A/B の役目**で、この観測は
「cap を低く置くと何を失いうるか」の所在を示すだけ。

### 判定基準（**A/B を回す前に確定させる** / 期待動作 3）

#171（`reviewer_effort_profile` の A/B）と同じ枠組みで決着させる。

**前提条件（満たさないなら A/B を回さない）**:

- 2 群は**同一世代**で揃える（#191。踏み下げ期は報告 0 件率 91% で recall 指標が死ぬ）
- 両群とも MAJOR 0 件なら**判定不能**として扱う。「指摘が減らなかった」を採用根拠にしない
  — もともと出ていない指標は減りようがない（#191 のコメントが指摘した自動通過の穴）
- 再試行で膨らんだ run は母数から外す（`agents_abandoned > 0` / #156 で総 `cache_read` の
  48% が破棄ぶんだった実測がある）

**判定**:

- **採用**: sub `cache_read` が **20% 以上**下がり、かつ **MAJOR 以上の指摘が 1 件も減らない**
- **不採用**: MAJOR 以上が 1 件でも減る（コスト削減幅は問わない）
- **保留**: 削減が 20% 未満（cap が実質的に効いていない ＝ 上限値を下げて再測）

20% の根拠は上の外挿で、**cap 50 で 18%**。これを下回る閾値だと「上限に届く体がほとんど
無い ＝ 実質何もしていない」設定でも通ってしまう。**閾値は cap が実際に噛む水準の上に置く。**

### 残っていること

- 期待動作 1（ロール別の往復上限を `prompts/reviewer-common.md` と explorer 側に置く）: **値が
  A/B 待ち**。なお `reviewer-common.md` の「探索予算」は**ファイル数と読む量**の上限で、
  往復回数の上限ではない。`explorer-common.md` には予算の節そのものが無い
- 期待動作 2（A/B の実施）: レビューを複数回まわす実験なので単発セッションでは完結しない
