...
- **反証レイヤーが覆っていない指摘では、一次ソース確認を load-bearing 判定の前に置く**（v2.105.0 / GitHub issue #196）。該当するのは `反証: 未実施` の回の全指摘と、`⚠️ 反証未実施（対象帯外）` が付いた指摘。**本節の他の条項は「load-bearing な事実主張」に絞って確認を求めるが、独立検証を一度も通っていない指摘では *どれが load-bearing か* の判定自体が指摘を書いた同じ 1 体の推論に乗っている** — 絞り込みの前提が担保されていない。実測でこの契約がある状態のまま失敗している（2026-08-25 / `fired=false, skip_reason=no-eligible-findings` の回の指摘を裏取りせず適用し、既存の不変条件と矛盾して回帰テストに止められた）
