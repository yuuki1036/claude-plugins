...
| `blocker_count` / `critical_count` / `major_count` / `minor_count` | severity 別件数（報告マトリクス通過後）。**4 つとも必須で、0 件でも省かない**（v2.111.0 / GitHub issue #215。`below_threshold_counts` と同じ文言）。歩留まり・報告 0 件率・真の空振り・`findings_class` 突合のすべての分子で、1 つでも欠けると publish が `payload:report_counts.missing` を立てて集計側がその回を母集団から外す ＝ **サンプルの損失**（実測: 欠測 7 件のうち 4 件は現行版の埋め落とし）。**トップレベルのフラットで書く** — `report_counts: {…}` / `counts: {…}` の入れ子は publish がトップレベルへ昇格して集計に載せる（v2.127.1 / GitHub issue #238。検知だけでは欠測が止まらず、同日 4 件中 2 件が入れ子で弾かれていた）が、`payload:report_counts.nested` が立ち規約違反として残る。昇格は 4 つ揃った入れ子だけで、部分欠測は救わない |
