...
>
> 同様に `duration_*` / `agents.explorer_waves` / `measurement_gaps` / `diff_digest` / `diff_files` / `tokens` / `machine_id` / `plugin_version` も**渡さない**（スクリプトが注入する）。
>
> **`inflated_axes` / `demoted_types` は必ず親オブジェクトの中に置く**（v2.107.0 / GitHub issue #208）。テンプレートの JSON は折り返しで親子関係が視覚的に切れるため、**トップレベルへ書いた回が実測で 5 件**あった。誤置すると `measurement_gaps` に `payload:<field>.misplaced` が立ち、**その値は集計されない**（publish は正しい位置へ移さない）。同様に **`agents` には契約の 7 キー以外を足さない**（動的層は `recall_skeptic` / `meta_reviewer` の専用フィールドで観測する。self-review の Markdown 推敲 agent も `md_polish` で観測し、`agents` に数えない）。契約外のキーは `payload:agents.vocab` が立つ。
>
> **層のオブジェクトそのもの（`pre_adjust_counts` / `adversarial_verify` / `recall_skeptic` / `meta_reviewer`。self-review は `md_polish` も）は必ず入れる。** 落ちた場合は版マーカーを注入する先が無いので、`measurement_gaps` に `payload:<field>` を立てて可視化する（空オブジェクトを捏造すると「起動記録なし」として母集団に混ざるため注入しない）。`fired` を落とした場合も同様に `payload:<field>.fired` が立つ。
...
- **`md_polish` は self-review だけのフィールド**なので、オブジェクトが無いときの `payload:md_polish` gap は **self-review の publish でだけ立てる**（review の publish で毎回立たないように。publish はプラグイン名で分ける）。版マーカーの注入は両 skill で同じ経路を通る
...
self-review 用（`--plugin code-review:self-review`）— **`pr` は `"local"` 固定 / `head_verified` を持たない / `comment_polish` を持つ / `md_polish` を持つ**、の 4 点だけが上との差:
...
```json
{
  ...
  "md_polish":{"fired":<bool>,"skip_reason":<string|null>,"suggested":<n>}
}
```
...
| `blocker_count` / `critical_count` / `major_count` / `minor_count` | severity 別件数（報告マトリクス通過後）。**4 つとも必須で、0 件でも省かない**（v2.111.0 / GitHub issue #215。`below_threshold_counts` と同じ文言）。歩留まり・報告 0 件率・真の空振り・`findings_class` 突合のすべての分子。**1 つでも欠けると publish は書き込まずに止まる**（v2.127.2 / GitHub issue #238。v2.111.0〜v2.127.1 は gap `payload:report_counts.missing` を立てて通していたが、検知だけでは欠測が止まらず現行版でも count キーそのものを持たない回が出続けた。止めて失う区間・トークン・dispatch の計測は、埋め直して再実行すれば同じ transcript から取れる — 打点ファイルは残る。`missing` の gap は旧版のイベントにだけ残る）。**トップレベルのフラットで書く** — `report_counts: {…}` / `counts: {…}` の入れ子は publish がトップレベルへ昇格して集計に載せる（v2.127.1 / GitHub issue #238。検知だけでは欠測が止まらず、同日 4 件中 2 件が入れ子で弾かれていた）が、`payload:report_counts.nested` が立ち規約違反として残る。昇格は 4 つ揃った入れ子だけで、部分欠測は救わない |
...
| `severity_threshold` | 実行時の `review_severity_threshold` 実効値（`BLOCKER`〜`MINOR`）。**どの severity が `## below-threshold` に回ったかを事後に判別するために必須**（v2.58.0〜）。これが無いと `pre_adjust_counts` の非可換性を補正できない。**publish が検証する**（v2.130.8 / #252）: 語彙外は fail-fast（黙って正規化しない — 正しい値は呼び出し側が知っている）/ トップレベルに無く `below_threshold_counts` / `pre_adjust_counts` の中に語彙内の値が 1 つに決まればトップレベルへ昇格して `payload:severity_threshold.nested` を立てる（実データの 5 件がこの形で、値はすべて `MAJOR`）/ どこにも無ければ `payload:severity_threshold` を立てて publish はする（他の分子は生きている）。retro は旧版で焼かれた入れ子も読み側で回収し、回収と欠落の件数を「計測の健全性」に出す |
...
| `machine_id` | **publish したマシン**（v2.120.0 / `publish-review-event.sh` が `hostname -s` から注入。**SKILL からは渡さない** — 渡しても上書きされる）。gist 集約（#61）のファイル名 `<hostname>.jsonl` と同じ値なので突合できる。取れなければ `null` + gap `machine-id`。**結果の読みがマシン間で食い違ったとき、どのマシンの回かを payload だけで言うため**（実測: 同じ issue への再集計が判定成立 51 件 / 42 件に割れ、出所をコメントから復元できなかった / #220） |
| `plugin_version` | **publish を実行した code-review の版**（v2.120.0 / 同上。`CLAUDE_PLUGIN_ROOT` ではなくスクリプト自身の位置の `plugin.json` から読む）。SKILL 本文と publish は同じ版ディレクトリから読まれるので、**その回がどの版の規約で走ったか**を表す。**配布ラグの交絡を外すための値** — 無いと「打ち手を入れた日以降」を日付で切るしかなく、`claude plugin update` 前のマシンで旧版のまま走った回が混ざる（#220 の留保）。取れなければ `null` + gap `plugin-version`。**これより前のイベントには無い**（日付やログのファイル名から推測して埋めない。retro は `未記録` に置く） |
...
- meta-reviewer / skeptic / Markdown 推敲 agent（self-review）は含めない（**それぞれ `meta_reviewer` / `recall_skeptic` / `md_polish` の専用フィールドで観測する**。`agents` は体数上限の効果測定用で、1 体固定の検証層や上限の外の層を混ぜると上限との対応が崩れる。**Markdown 推敲 agent を `reviewer` に含めると、突合で `md_polish.fired` と二重に数えて `agents-mismatch` が立つ**）
...
- 「本文を書いてから捨てた」が**負になる回がある** — 報告件数が本文を書いた数を超えた回（`payload:report_counts.exceeds-body`）。**丸めない**（0 に丸めると「捨てていない」と読める）。旧版はこれを「手順 1 の後に走る層（`recall_skeptic` / `meta_reviewer` の `findings_added`）が指摘を足すため」と説明していたが、両 SKILL は skeptic（5.8 / 4.8）と meta（5.6 / 4.6）を**手順 1 より前**に統合するので、その指摘は `pre_adjust_counts` に既に入っている（v2.130.9 / #248 で確認）
- **「本文を書いた数」は下限として読む**（v2.130.9 / #248）。契約 (a)（scoring-guide.md `## 報告閾値を割った指摘の記録`）では閾値未満に本文を書かないので、1 回ごとに `報告 ≤ pre − below`・`付録の行数 ≤ max((pre − below) − 報告, 0)` が成り立つ（skeptic / meta の `findings_added` は足さない — 上の理由で pre に入っている）。付録が超えた回（既存データで 89 回中 43 回。opus-4-8 は 54 回中 37 回）は閾値未満に数えた指摘にも本文が書かれていた。publish は `payload:appendix.exceeds-body` / `payload:report_counts.exceeds-body` を立て、retro は層ごとに上限超えの回数（付録 / 報告件数の別）と、付録と報告から見た下限 Σmax(pre − below, 付録 + 報告) を並べる。**違反した回を母集団から外さない**（opus-4-8 では大半が消える）


**`missing_coverage` の記法（v2.44.0 で語彙固定・v2.66.0 で機械検証）** — 要素は **識別子のみ**とし、理由・件数・finding id・自由文を混ぜない:

- 許容形: `<focus 名>`（例 `performance` / `error-handling`）/ `<phase 名>`（例 `explorer` / `round2` / `recall-skeptic` / `meta-reviewer` / `adversarial-verify` / `md-polish`）/ `<phase 名>:<focus 名>`（例 `explorer:value-flow-trace` / `head-mismatch:security`）。**正規表現 `^[a-z0-9-]+(:[a-z0-9-]+)?$` に完全一致**（`re.fullmatch`。`re.match` だと末尾改行 1 個を通すので使わない）
...
- `fired` / `skip_reason`: **発火記録**（v2.65.0 / GitHub issue #129）。他の 2 つの動的層（`recall_skeptic` / `meta_reviewer`）は持っていたのに**この層だけ持たず**、下流は `agents.verify > 0` から起動有無を推定するしかなかった。それでは **「走らなかった」と「走れる対象が無かった」を区別できない**:
  - `skip_reason` の語彙は `"effort"`（low / medium）/ `"config"`（`enable_adversarial_verify: false`）/ `"scope"`（self-review の `--focus` / `--exclude`。`--exclude md-polish` 単独は含まない）/ `"emergency"`（`--emergency` / `skip-mode`）/ **`"no-eligible-findings"`**（triage-dynamic-gates.md `## 9` のゲートに合致する指摘が 0 件）。`fired=true` なら `null`
  - **`"no-eligible-findings"` が本フィールドの主目的**。既定 effort（high）のゲートは非対称ゾーン（BLOCKER 60-94 / CRITICAL 80-94）だけなので、**BLOCKER / CRITICAL が 1 件も出なければ MAJOR がいくら出ても対象は構造的に 0 件**になる（実測: 本リポジトリの `pre_adjust_counts` を持つ 6 件中 3 件が不発で、いずれも BLOCKER + CRITICAL = 0・MAJOR は 6〜8 件。**「3 回連続」ではない** — 間に起動回が 2 件挟まる。issue #129 が別途「3 回連続」と報告しているのは複数リポジトリを含む実運用の体感で、この 6 件表からは再現しない）。他の 4 つと違い**設計上の非該当ではなく「ゲート幅が実効的に狭いか」の観測点**なので、下流の分母から外さない（`review-retro.sh` の `OUT_OF_SCOPE_SKIPS` に入れない）。**retro は率を effort 帯別（high / xhigh 以上）に出し、⚠️ にはしない**（v2.130.12 / GitHub issue #249。ゲートの定義が effort で違うので、1 つの分母に混ぜると effort の構成差が世代差に見える。読み方は `triage-dynamic-gates.md ## 9` の再監視条件）
...
**`md_polish`**（**self-review のみ** / v2.131.0 / GitHub issue #243）— Markdown 推敲（`md-polish-guide.md` / `prompts/md-polish.md`）の実行記録。**動的層の 3 点セット（`fired` / `skip_reason` / `gate_schema`）を持つ**:

- `fired`: 推敲 agent を起動したか（bool）。**起動しなかった回もオブジェクトごと入れる**（落とすと self-review でだけ `payload:md_polish` が立つ）
- `skip_reason`: `fired=false` のときの理由。上から評価して最初に当たった値（正本の判定表は `md-polish-guide.md` の 1 節）: `"no-md-prose"`（推敲してよい md の追加・変更行が無い）/ `"embed"`（`--embed`。適用する Step 7 が無い）/ `"scope"`（`--focus` 指定、または `--exclude` に `md-polish` を含む）/ `"not-installed"`（writing-polish が有効でない）。`fired=true` なら `null`。**effort は起動条件に関与しないので `"effort"` は無い**。**4 つとも設計上の非該当**で、retro は発火率の分母から外す（「対象が 0 件」に当たるのは `no-md-prose` だが、この層は md の変更がある回にだけ意味を持つので分母に残さない）
- `suggested`: 提案の件数（**二重掲載の除去の後・掲載上限で切る前**。Step 7 の適用対象と同じ集合）。agent が失敗した・`## Markdown 推敲提案` の見出しが無い回は **`-1`（測定不能）**とし、`missing_coverage` に `md-polish` を記録する（`comment_polish.suggested` と同じ規約）。`fired=false` なら `0`
- `gate_schema`: **起動ゲートの版**。**`publish-review-event.sh` が注入する**（1 = effort を問わず md の散文が 1 行以上・writing-polish 有効で起動する v2.131.0 以降）
- **申告体数の突合に入る**: 推敲 agent は transcript に 1 体として現れるので、publish は `md_polish.fired` を申告体数に足してから `dispatch.agents_completed` と比べる（`meta_reviewer` / `recall_skeptic` と同じ補正。`review-backfill.sh` と `review-retro.sh` の体数も同じ式）。**reviewer wave に相乗りするので期待 wave 本数の項は足さない** — 別メッセージで出した回に `wave-split` が立つのは正しい挙動
- **適用件数は載せない**。publish（Step 6.4）は適用（Step 7）より前なので測れない（`comment_polish` と同じ）
- review 側は publish しない（self-review 限定）
...
| `session_source` | transcript の選び方（v2.130.6 / #246）。`"env"` = `CLAUDE_CODE_SESSION_ID` から引いた。**無い回は旧版の「候補 dir の最新 `.jsonl`」推定**で、retro は取り違え疑いを判定して外す（下記） |
...
- **transcript は `CLAUDE_CODE_SESSION_ID` から引く**（v2.130.6 / #246。引き方の正本は `lib/review-paths.sh` の `review_session_transcript` で、Phase 0 の世代表示と共有）。旧版は「候補 dir の最新 `.jsonl`」という推定で、**publish 前に `cd` した回**（worktree ⇄ main root / 別リポジトリ）は候補 slug に自分が居ないので欠測し、**同じ slug に並行セッションがある回**は cwd が合っていても新しい方の別セッションを採った。後者は値がもっともらしく gap にも出ないので欠測より害が大きい。引けない回は推定に倒さず `session-unresolved` を立てる
- **どの transcript のどこからを数えたかは `session` / `first_ts` に残す。** 値そのものはもっともらしいので、**この 2 つが無いと取り違えを事後に検出する手段が消える**。retro は `session_source` を持たない回に 2 つの規則を当て、該当した回の `tokens` / `models` / `dispatch` を集計から外す — ① 同じ `session` で `since-t0` 系の窓（`first_ts` から publish まで）が重なる 2 回（推定で引いた側すべて。どちらが正しい側かは payload だけでは決まらない。id で引いた回は相手としてだけ数える。`session` 窓は `first_ts` がセッションの先頭で、同じセッションの正常な 2 回でも重なるので突合に入れない）② explorer / reviewer / specialist / verify を起動したのに `sub_agents` が 0（`sub_agents` を持たない回は `models.sub_distinct` が空）。**同じ計測から publish が導いた gap**（`tokens` / `tokens-sub` / `models` / `dispatch` / `agents-*` / `wave-split` / `fleet-span-mismatch`）と、打点を補完した回（`derived_markers`）の triage / explore / fleet / synthesis 区間も一緒に外す。既存データで混入 3 件を全部拾い、誤検出は ① の巻き添え 1 件だけだった。外した件数は「計測の健全性」に出る
- **`main.n == 0` は「トークンが 0 だった」ではない。** review は必ずメインループのメッセージを出してから publish するので、0 は「窓が空振りした」（または計測の失敗）を意味する（transcript を引けなかった回は v2.130.6 から `session-unresolved` に分かれた）。この回は `tokens` を載せず `measurement_gaps` に `tokens` を立てる（ゼロを実測値として載せると retro の中央値と体数相関が壊れる。実測で相関が 1.00 → 0.18 に落ちた）
...
- 版マーカー: **`duration_triage_min` の存在が v2.41.0 以降・`duration_explore_min` の存在が v2.43.0 以降・`pre_adjust_counts` の存在が v2.44.0 以降（**算出方法の版は `pre_adjust_counts.schema`**）・`comment_polish` の存在が v2.45.0 以降（self-review のみ）・`md_polish` の存在が v2.131.0 以降（self-review のみ）・`severity_threshold` の存在が v2.58.0 以降・`duration_synthesis_min` の存在が v2.60.0 以降（**meta の起動ゲートの版は `meta_reviewer.gate_schema`**）・`agents.explorer_waves` の存在が v2.61.0 以降・`measurement_gaps` / `diff_digest` の存在が v2.62.0 以降（**上流 severity 較正の版は `adversarial_verify.calibration_schema`**）・`adversarial_verify.fired` / `tokens` の存在が v2.65.0 以降（**反証の起動ゲートの版は `adversarial_verify.gate_schema`**）**。層別は必ずフィールドの有無で行い、日付では切らない。**v2.43.0 未満の `duration_*` は並行セッション汚染を受けうる**（issue #99）ためロールバック判断の基準側に使わない
...
> **worktree 内から実行しても引数なしで通る**（GitHub issue #112）。transcript の slug はセッションを**開始した**ディレクトリ由来なので、review 経路（Step 0 で必ず `EnterWorktree`）では cwd 側の slug にメインループの transcript が存在しない。スクリプトは **cwd 側とメインリポジトリ側（`--git-common-dir` 由来）の両方**を候補にして最新の `.jsonl` を採るので、review 後にそのまま実行してよい。dev-workflow の作業用 worktree 内で開始したセッション（transcript が cwd 側にある逆パターン）も同じ仕組みで拾える。**どちらの候補にも無いときは `--session <絶対パス>`** を使う（`--list` が探索したディレクトリを表示する）。**この推定は並行セッションがあると外れる**（同じ slug で新しい方の別セッションを採る）。publish は推定を使わず `CLAUDE_CODE_SESSION_ID` から引いた transcript を `--session` で渡す（v2.130.6 / #246）。手動実行でも自分のセッションを確実に測るなら同じく `--session` を渡す。
...
| 出力 | 戻り先 |
|---|---|
| effort × size_tier の fleet / 体数、体数 vs fleet の相関 | triage-guide.md `## 7`（**体数を壁時計のレバーとして扱わない**）。**結論文は `r` と n で分岐する** — n < 10 は解釈しない / `|r| < 0.3` は上記主張を支持 / 0.3〜0.6 は effort の交絡を疑って帯別に見る / `|r|` 0.6 以上は表に「高い（既知 / #217）」と出るが**⚠️ には出さない**（v2.116.0 で再監視を終了 — 相関はあるが因果ではなく、規範の根拠は recall 側へ移した。層別キーは tier × 世代）。**低いことも高いことも、それ単独では打ち手にならない**（#217） |
...
> **下限は世代の層ごとに適用する**（v2.109.0 / GitHub issue #209）。母集団の違う層を平均すると崩れている層が薄まって鳴らない（実測: 反証レイヤーの不発が opus-4-8 で 78% あったのに、累計 43% では閾値の 50 に届かなかった）。**閾値を超えた層が 1 つも無ければ累計で評価し、その旨を行に添える** — どの層も下限に届かない期間は珍しくなく、層別だけにすると**いま鳴っている行が消えて「該当なし」と読まれる**。分岐を「判定できた層の有無」で切らないこと（判定できた層が健全なだけのとき、下限未満の層が崩れていても行が 1 本も出なくなる）。下限に届かないが閾値を超えている層は ⚠️ ではなく「計測の健全性」に**判定の保留**として出す（⚠️ に出すと「行動する根拠がある」という意味が壊れる）。保留行には**あと何件で判定可能か**（下限 − 層の分母）を添え、`--json` の `pending` にも全層を出す（v2.123.0 / GitHub issue #230。版で絞った窓では保留が主要な出力になり、件数が無いと読み直す時期が決まらない）。
>
> **真の空振り率**（v2.112.0 / GitHub issue #214）: `appendix` を持つ回のうち **報告 0 件かつ付録推奨 0** の割合（#168 の「付録に救われた回は空振りではない」を差し引いた値）。#210 の判定基準「20% 未満で回復」の片方で、世代で層別して ⚠️ に載せる（下限 10）。`報告 0 件率` は付録に救われた回を含む粗い代理、こちらが本命。**内訳を「検出 0」と「検出はあったが全部閾値未満」に割って表と ⚠️ の両方に出す**（v2.117.0 / GitHub issue #210）— 打ち手が違うため。前者は reviewer が何も見つけていない（recall の問題）、後者は見つけたものが `## below-threshold` に件数だけ返り報告にも付録にも出ない（閾値と付録の方針の問題。付録の対象は「reviewer が列挙した指摘」だけという契約 / scoring-guide.md）。`pre_adjust_counts` を持たない回・語彙違反の回は `unknown` に置く（0 に丸めると「検出が無かった」に化ける）。**実測 opus-4-8 n=21: 真の空振り 9 件のうち検出 0 は 2 件だけ**。**率そのものは絞らない**（報告 0 かつ推奨 0 は「何も返ってこなかった」割合で利用者の体感そのもの）。**内訳で分岐するのは ⚠️ の処方の側**: 検出 0 が主なら実行世代、閾値未満のみが主なら閾値と付録の方針、同数・判定不能なら絞らず両方を出す
...
- **`agents` の突合式は `publish-review-event.sh` の `declared` と同一にする**（allow-list の 5 キー + `recall_skeptic` / `meta_reviewer` / `md_polish` の `fired`）。deny-list で書くと契約外のキー（実データに `skeptic` の例がある）を足してしまい、#154 が検知したい信号と同じ形のノイズになる
