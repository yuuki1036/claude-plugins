...
| `dispatch` | **agent の発行パターン**（v2.70.0 / GitHub issue #142 / `publish-review-event.sh` が `measure-tokens.sh --json` から注入。**SKILL からは渡さない**）。`{agents, max_gap_sec, span_sec, verdict}` で、`verdict` は `batched`（同一メッセージ内の一括発行）/ `serial`（1 体ずつ別メッセージ）/ `mixed`（何回かに分割）/ `single`（1 体で判定不能）。**`duration_fleet_min` だけでは「9 体を逐次で回した 89 分」と「1 体が 89 分かかった」を区別できない**。判定は agent transcript の起動時刻の間隔（閾値 120 秒）で、LLM の自己申告ではない |
