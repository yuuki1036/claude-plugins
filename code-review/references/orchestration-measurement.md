...
>
> **真の空振り率**（v2.112.0 / GitHub issue #214）: `appendix` を持つ回のうち **報告 0 件かつ付録推奨 0** の割合（#168 の「付録に救われた回は空振りではない」を差し引いた値）。#210 の判定基準「20% 未満で回復」の片方で、世代で層別して ⚠️ に載せる（下限は反証の不発と同じ 10）。`報告 0 件率` は付録に救われた回を含む粗い代理、こちらが本命。
