...
- **中止も確認もしない。警告だけ出す**（v2.110.0 / #210 候補 1a）。`models=` が **4 系世代**
  ...
  - **内訳（v2.117.0 / #210）: 真の空振り 9 件のうち検出 0 は 2 件だけで、残り 7 件は MINOR を
    2〜6 件見つけている**（n=21 全件で閾値は MAJOR）。4.8 は検出が空なのではなく、
    **検出の 88%（111/127）を MINOR に置く** — opus-5 は 67%（149/223）。閾値 MAJOR の下では
    MINOR のみの回は設計どおり報告 0 になり、`## below-threshold` に件数だけ返った分は
    付録の対象外（scoring-guide.md）なので付録にも出ない。**「世代を上げる」以外に
    「4.8 の帯では閾値・付録の方針を変える」という打ち手がある**ことがこの内訳で分かる
  - **⚠️ の処方は内訳で分岐する**（v2.118.0 / #210）。率（20% 未満）は据え置きで、
    「実行世代を見直す」と出るのは**検出 0 が主の層だけ**。閾値未満のみが主なら
    「閾値と付録の方針」を指し、**世代を上げても同じ結果になりうる**と明示する。
    同数・判定不能のときは打ち手を 1 つに絞らない（推測で片方へ倒さない）。
    **率の分母を「検出 0」だけに絞る案は採らなかった** — 基準は満たせるが、
    データに合わせて定義を動かして基準を通す形（#217 で却下したのと同型）になる
