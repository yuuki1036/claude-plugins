...
- **体数を壁時計のレバーとして扱わない**。並列発行が効いている限り fleet 区間の実時間は「wave 内最長の 1 体」で決まるため体数削減の効果は線形ではない。**実測でも体数と `duration_fleet_min` は無相関で、支配的なのは effort（= 直列 wave 数）だった**（review 13 件。`size_tier` を medium に揃えると high 平均 32 分 / xhigh 平均 61 分と **1.9 倍**なのに、体数レンジは 6〜10 と 6〜11 でほぼ重なる。`73 分 / 6 体` と `19 分 / 7 体` が併存する。GitHub issue #116 / 内訳: `design-notes/triage-rationale.md`）。体数削減が確実に効くのは**トークンコスト**。壁時計を縮めたいときにまず触るのは ①1 体あたりの探索量（`prompts/reviewer-common.md` の探索予算）②直列 wave 数（`## 5.1`）③メインコンテキストの複製量（orchestration-guide.md `## 3.5`）。**この節のロールバック判断に「時間が長いから体数を減らす」を混ぜない**（recall だけ落ちて時間が変わらない改悪になる）

### 7.1 reviewer effort profile（A/B 実験・userConfig `reviewer_effort_profile`）

reviewer の model effort を focus 別に差別化できる実験フラグ。**既定は `uniform`（現行挙動）で下流に一切影響しない**。「Opus 5 の素の性能なら低密度観点は `medium` でも recall が落ちないのでは」という仮説を計測で検証するための A/B スイッチ（手順・判定基準・撤去条件は `design-notes/pending-optimizations.md`）。effort が cache（往復×文脈量）ではなく output/thinking 側（コスト全体の ~17%）に効くため**節約は modest** と分かったうえで、recall を落とさず取れる分だけ取る、という位置づけ。

| profile | reviewer effort |
|---|---|
| `uniform`（既定） | `## 7` 上表どおり（high 帯 → `high` / xhigh・max → `xhigh`）。変更なし |
| `differentiated` | **high 帯でのみ**、下記「高密度観点」は `high` を維持し、それ以外を `medium` に下げる。**xhigh / max は明示 escalation なので profile を無視して全 reviewer を `xhigh` のまま**（深さを買う意思表示を尊重する） |

- **`high` を維持する高密度観点**: 束ねず単独維持の 4 観点（bug-detection / security / spec-compliance / claude-md-compliance）＋ error-handling / migration / performance（正しさ・データ整合・障害直結）。**specialist は全 focus で `high` 維持**（BLOCKER 判定の深さが役割そのもの）
- **`medium` に下げる低密度観点**: 上記以外（comment-accuracy / comment-polish / pattern-consistency / config / dependency / type-design / ui-quality / cross-cutting / doc-substance / test-quality / api-design）
- **検証層（meta / skeptic / 反証）と explorer は profile の対象外**（explorer は既に sonnet、検証層は 1 体固定で effort=recall/precision そのもの）
- 最小保証の 2 体（bug-detection / claude-md-compliance）は高密度側なので `differentiated` でも `high` のまま
- profile は `review:completed` payload の `reviewer_effort_profile` に記録し、A/B の arm を事後に層別できるようにする（orchestration-measurement.md `## 16`）
