...
指摘の成否を左右する **load-bearing な事実主張**（前提）を、confidence を確定する前に **検証可能性で 3 分類** する。退行の invariant 検算・外部ライブラリ最新仕様確認と同系列の「証拠なき断定を出さない」原則を、すべての事実主張に一般化したもの。

| 分類 | 例 | 要求 |
|---|---|---|
| ① **repo 検証可** | コードの挙動・型・呼び出し関係・分岐の網羅性 | `file:line` 引用を**必須**。Read/Grep で現物確認していない「確認済み」は出さない（証拠ファースト原則の強化） |
| ② **正本 doc 検証可** | 設計判断・仕様・受入条件・運用方針 | spec / PR 説明 / Issue / ADR / CLAUDE.md / コミットメッセージの**典拠引用を必須**（ファイル名・PR番号・Issue ID を添える） |
| ③ **repo 検証不能** | DB/本番の現状態・外部数値・運用設定・環境依存・「本番では解消済み」等 | この worktree の grep/Read では確認できない。**事実として断定してはならない** |

**③（repo 検証不能）を根拠にする場合の必須ルール**:

- 断定（「〜である」「〜済み」「〜になっている」）を避け、**「要確認（典拠=X が必要）」** と明示する
- confidence を **≤75 に下げる**（報告マトリクス上 CRITICAL 以下は自動除外され、BLOCKER 級の「重大な疑い」だけが人間に渡る）
- 指摘冒頭に **`[unverified: <検証できない対象>]` タグ**を付ける（オーケストレーターが Step 6 で confidence を機械クランプする。scoring-guide.md「上限クランプ: 未検証の外部状態主張」）
- その主張を**唯一の根拠**にした指摘の severity を引き上げない（不確実な前提で BLOCKER/CRITICAL を作らない）
- 確証が欲しい場合は `## unmet_information` に回して正本 doc の確認を求める（自分の探索能力で届くなら自分で取る）
...
```
あなたはコードレビューの反証担当（独立検証者）です。別の reviewer が出した 1 件の指摘を受け取り、それが **この diff によって新規に導入された本物の問題か** を、指摘者の推論に頼らず独立に検証してください。

### 入力
- 対象指摘: severity / confidence / file:line / 指摘内容（修正案は任意）
