...
| xhigh | 起動（2 段） | 起動（**`small` 帯は BLOCKER 有りのみ**） |
| max | 起動（2 段） | 起動（**`small` 帯は BLOCKER 有りのみ**） |

**Phase 5.6 だけが規模帯に連動する（v2.60.0）**。他のフェーズ（5.5 / 5.8 / 5.9）は従来どおり帯に連動しない — triage-guide.md `## 6.3` の「規模キャップが削るのは breadth だけ」という原則の**唯一の例外**で、根拠が n=1 と弱いため**ロールバック条件つきの暫定措置**として入れてある（`design-notes/triage-rationale.md`）。他フェーズへ同じ帯連動を横展開しないこと。
