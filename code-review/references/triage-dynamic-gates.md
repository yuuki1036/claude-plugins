...
<!-- SSOT: code-review/references/orchestration-guide.md#5 @f7c3afba -->
...
**再監視の条件**（v2.130.12 / GitHub issue #249 で改めた）: `review-retro.sh` は反証の不発（`no-eligible-findings`）を **⚠️ にせず**、「動的層の発火」に **effort 帯別**（high / xhigh 以上）の不発率を出す。以前は世代の層ごとに 10 件・50% 以上で ⚠️ を出していたが（v2.109.0 / #209）、分母に**ゲートの定義が違う effort の回が混ざっていた**。high 帯は世代が分かる 31 回中 30 回が不発で（retro / gist 集約 n=280 / 2026-09-26）、どの世代も 9 割超だが、その 30 回は「上流に MAJOR も無かった」17 回と「MAJOR はあったが帯の外」12 回（残り 1 回は上流の件数が欠測）の和で、**率からはどちらかを読めない**。xhigh 以上の不発は報告見込みの指摘 0 件に近く、報告 0 件率の表と同じものを測る。あわせて retro は、xhigh 以上の発火回のうち**反証にかかったのが MAJOR だけの回**の verdict を出す（再検討条件の後半の代理。定義は `design-notes/scoring-rationale.md`）。**ゲート幅の再検討は率ではなく `design-notes/scoring-rationale.md` の再検討条件で行う**。検討の選択肢は「high の非対称ゾーンに MAJOR の一部帯を足す」だが、**足すこと自体が結論ではない** — 上表の設計意図（「詰めると取り下がるのは不確実だが報告される非対称ゾーン」）と、下の xhigh / max 節が記録しているトレードオフ（最も取り下がりにくい層に wave 1 本を使う）を併せて判断する。

- **狭いこと自体はまだ問題だと言えない。** 本 issue が直したのは**測れないこと**であって、ゲート幅ではない
- **再検討した結果、今は広げない**（v2.105.0 / GitHub issue #196 期待動作 2）。当時の理由は「不発率が世代と交絡している」だったが、**交絡していたのは effort だった**（v2.130.12 / #249）: high 帯は世代を揃えても不発率が 9 割超で動かない（opus-4-8 18/19・opus-5 2/2・opus-5-5 10/10）。**率だけでは帯の狭さと上流の産出量を分けられないが、`pre_adjust_counts` と突き合わせると回ごとに分けられる** — 不発 30 回のうち 12 回は上流に MAJOR があって帯に入らなかった（帯の狭さ）、17 回は上流の MAJOR も 0 件だった（帯を広げても不発）。それでも広げない理由と、再検討条件の後半（MAJOR の偽陽性）を xhigh 以上の verdict で代理観測した結果は `design-notes/scoring-rationale.md`
...
- **「加減算で報告閾値を超えた MAJOR」を high の対象に足す案は保留**（#136 の本命案）。`scoring-guide.md` の「複数エージェント検出 +15」は MAJOR を単独で 95 へ押し上げられるのに押し上げの妥当性が未検証、という指摘自体は妥当だが、**既定 high では meta が走らないため反証 wave がそもそも無い回が多く、足すと直列 wave が 1 本生える**（xhigh / max は既に MAJOR 全件が対象なので効くのは high 限定）。判断は `design-notes/scoring-rationale.md` の再検討条件と併せて行う → `design-notes/pending-optimizations.md ## 7`
