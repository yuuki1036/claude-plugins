...
| ③ | **最初から閾値未満** — 反証も加減算も受けず、reviewer の付けた (severity, confidence) のままマトリクスを通らない（例: spec-compliance が 70〜80 で出した MAJOR） | 手順 1（reviewer の自己較正） | **本節で追加**。reviewer は severity の閾値でしか列挙を絞らないので、MAJOR の 60〜94 はこの経路で落ちる |
...
記録形式は各 SKILL.md の Step 6 レポートテンプレート（`### 🔁 報告閾値を割った指摘`）に従う。**脱落理由には経路が①〜③のどれか分かる情報を必ず入れる**（①は verdict 名と反証根拠 file:line、②は適用した規則名と confidence の遷移、③は「調整なし」と reviewer の付けた値）。③が続く観点は、その観点のプロンプトの較正がマトリクスと噛み合っていない兆候として読む（spec-compliance の較正を直した実例がある）。

### 推奨マーカー（`※ 推奨:`）— 「報告 0 件」と「価値 0」を分ける（GitHub issue #168）

**閾値の外に落ちたが人間には見てほしい指摘には、その項目に `※ 推奨: <理由>` を 1 行足す。**

付録に列挙しただけでは、集計側からは**報告 0 件の回と価値 0 の回が同じ形**に見える。実測（`2026-08-24T02:34:04Z` / PR 398 / 21 体 / 62 分）では severity 4 バケツすべて 0 なのに付録に 18 件が並び、うち 4 件は「この diff 自身のファイルに 1 行で閉じるので見送るのは惜しい」と本文で人間に推されていた。1 件は反証エージェントが実際にコードを削ってテストを走らせ、**テストの穴が実証済み**だった。この回は空振りではないのに、payload 上は空振りと区別がつかない。

- **理由は 1 件ずつ具体で書く**。「重要そう」では書けない粒度にすること自体が件数の歯止めになる（上限は設けない — **数えられるようにしてから実測で決める**。全件が推奨に膨らむなら `recommended / listed` の比率が先に表に出る）
- **推奨の判断軸は「修正コストが小さいか」**。閾値を割った理由（confidence が足りない / severity が低い）は既に脱落理由の欄にあるので、ここで繰り返さない。書くのは**なぜ見送るのが惜しいか**（この diff 内で閉じる / 1 行で直る / 実証済みのテスト穴、など）
- **総括には再掲しない**。同じ事実を 2 箇所に書くと drift する。人間は付録を読む
- payload には `appendix: {listed, recommended}` として載せる（`orchestration-measurement.md ## 16`）。**`recommended` は `※ 推奨:` を付けた行数**で、`listed` を超えることはない（publish が fail-fast する）
...
- **足した件数は `below_threshold_counts` にも再掲する**（同 `## 16` / GitHub issue #146）。合算だけでは **(a) 本文を書いてから捨てた**（出力トークンの純損失）と **(b) 件数だけ返した**（既に節約できている）が分離できず、**閾値注入が効いているのかどうかを判定できない**（実測: 検出 342 → 報告 91 = 26.6% の内訳が読めなかった）
- **抑制は列挙だけで、判定は従来どおり**。閾値未満を理由にした severity の繰り上げは較正の破壊として扱う（reviewer 側にも明記済み）

### 閾値未満の件数通知 — 報告 0 件の回に限る（GitHub issue #242）

**報告が severity 4 バケツすべて 0 件で、かつ `below_threshold_counts` の 4 バケツ合計が 1 件以上の回は、レポートに件数通知を 1 行出す。** 書式は各 SKILL.md の Step 6 レポートテンプレート（`### 🔎 閾値未満の候補`）。

- 件数だけ返った閾値未満は 🔁 付録の対象外なので（`## 報告閾値を割った指摘の記録`）、この通知が無いと **「reviewer が何も見つけなかった回」と「見つけたが全部閾値未満だった回」がユーザーから同じ形に見える**。実測（2 マシン合算 n=259）: opus-4-8 層の真の空振り 17 件のうち 14 件が後者（版で絞った窓でも 8 件中 7 件）
- **出すのは件数と severity の内訳だけ**。中身は reviewer が書いていないので列挙しない（全件書き起こす案は、上の「件数だけ返す」節約を壊すので採らない）
- **条件はどちらも Step 6 で payload に埋める値から決まる**（報告件数 4 フィールド / `below_threshold_counts`）。判断を挟まない。報告が 1 件でもある回は出さない（本文を読めば検出があったことは伝わる）
- 🔁 付録が載っている回でも条件を満たせば出す。付録と閾値未満は別の母集団で、付録の件数から閾値未満の件数は読めない
