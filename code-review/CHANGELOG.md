...
- **retro の反証不発を ⚠️ から外し、effort 帯別（high / xhigh 以上）の不発率として「動的層の発火」に出すようにした**
  （#249）。以前は世代の層ごとに 50% 以上で「既定 high のゲート幅を再検討」の ⚠️ を出していたが、分母にゲートの定義が
  違う effort の回が混ざっていた。high 帯は世代を揃えても 9 割超が不発で、率は「上流に MAJOR も無かった」回と
  「MAJOR はあったが帯の外」の回の和なので、率からは行動が決まらない。xhigh 以上の不発は報告見込みの指摘 0 件に近く、
  報告 0 件率の表と同じものを測る。現データでは ⚠️ が 2 本（opus-4-8 75% / opus-5-5 100%）減る
- **xhigh 以上の帯の下に「報告見込みの MAJOR への verdict」を出すようにした**（#249）。上流に BLOCKER / CRITICAL が
  0 件で閾値が MAJOR の発火回は、反証にかかったのが MAJOR だけなので、high では反証を通らない「報告された MAJOR」の
  代理になる。現データは 26 回・124 件で refuted 8% / severity_inflated 72%
- **反証ゲートを広げない理由の「世代と交絡」を「effort と交絡」に直し、再検討条件の後半を代理で観測した結果を書いた**
  （`scoring-rationale.md` / `triage-dynamic-gates.md ## 9`）。high の不発 30 回は、上流に MAJOR があって帯に
  入らなかった 12 回と、上流の MAJOR も 0 件だった 17 回に分かれる。ゲートを広げるかは判断を保留し、
  `pending-optimizations.md ## 7` は据え置く
...
- **retro の上流較正の節が、世代欠測（`unrecorded` / `mixed`）を 1 世代として代表に選び、世代の数え上げにも入れていたのを
  直した**（#251）。版窓（`--min-plugin-version`）では欠測した 1 回が代表になり、`2.128.0` 以上では「層 3 は世代 unrecorded
  にしか存在しない・達成不能」と誤って出していた（実際は opus-5-5 が層 3 を 14 回走って反証が 1 度も発火していなかった）。
  代表は最新の calib を走った既知世代から verdict 数 → 層キーの順で決定的に選ぶ。表の行は残す
- **verdict 0 件で最新の層を走った世代も黙らせない**。世代ごとの走行回数・反証の発火・skip 理由と、世代を記録できなかった
  回の件数を節に出す。⚠️ の文言は「層は X にしか存在しない」ではなく「層の verdict は X にしか無い」にした

### Changed

- **最新より古い calib の層では `severity_inflated` の ⚠️ を鳴らさない**（#251 提案 3 を採用）。publish は現行の
  `calibration_schema` を固定で注入するので古い層はもう増えず、鳴っても行動の先が無い（#221 で「鳴っていること自体は
  新しい情報ではない」と確認済み）。古さは母集団の最新と比べるので、最新が古い calib しか無い母集団では従来どおり鳴る。
  世代が分からない層（`unrecorded` / `mixed`）でも鳴らさない（節が「効果判定に使わない」と言う層なので）。
  累計の ⚠️ は 2 本から 1 本になる

## [2.130.9] - 2026-09-26

### Changed

- **閾値未満の指摘に本文を書かない・付録に載せない、を契約として決め直した（契約 (a) / #248）**。reviewer が
  `## below-threshold` の件数行の後や独自の「明細」節に本文を書き、オーケストレーターが below に数えたうえで 🔁 付録にも
  載せる回があった（付録が本文を書いた数の上限を超える回が 89 回中 43 回。opus-4-8 は 54 回中 37 回）。reviewer 規約・
  両 SKILL の Step 6・scoring-guide の付録節に明記した。本文を認めて数える案 (b) は、真の空振り率がこの混入の増減だけで
  動く（opus-4-8 で 33% から最大 60%）ので採らなかった

### Added

- **publish が付録・報告件数の上限超えに gap を立てる**（`payload:appendix.exceeds-body` /
  `payload:report_counts.exceeds-body`）。上限の式は `scripts/lib/body_bound.py`。WARN も fail-fast もしない。
  skeptic / meta の `findings_added` は上限に足さない — 両 SKILL は手順 1 より前に統合するので `pre_adjust_counts` に
  既に入っている（`## 16` の「手順 1 の後に走る層が足すので本文を書いてから捨てたが負になる」という旧説明も直した）
- **retro が上限超えを契約どおりに読む**: 検出内訳に層ごとの上限超え回数と「付録と報告から見た本文を書いた数の下限」を
  並べる（opus-4-8 の書いた後の破棄率は 60% → 87% 以上）/ 真の空振りの「閾値未満のみ」を付録の有無で分け、⚠️ の処方文が
  「付録にも出ていない」と言い切るのを付録に無い回だけにし、「契約外」と言うのは付録が上限を超えた回だけにする /
  推奨で救われた回のうち上限 0 の回（契約外の行に救われた回）を
  数える。上限超えのマーカーは欠測ではないので欠測内訳と ⚠️ から外す
...
- **test-quality の起動条件を広げ、アンチパターンを名前で指摘させるようにした**。severity の目安は「テスト欠如」を
  重く見るのに、起動条件がテストファイルの変更に限られていたので、テストを書かずに足したソースでは reviewer が
  起動しなかった。テスト以外の新しいソースファイルがあるのにテストの変更が 0 の回も起動する（設定・ドキュメント・
  型定義だけの新規ファイルは数えない）。reviewer には、実装に結合したテスト・同語反復（期待値をコードと同じ計算で
  作る）・システム境界の外での mock を名前付きで挙げ、同語反復は「テスト欠如」と同じ重さで扱わせる
  （翻案元: mattpocock/skills の tdd `tests.md`）

## [2.130.1] - 2026-09-24

### Fixed

- **spec-compliance の確信度の付け方が報告マトリクスと噛み合わず、スコープ逸脱と設計判断との矛盾（どちらも MAJOR）が本文に出なかった**。
  プロンプトはスコープ逸脱を confidence 70-80 と定めていたが（2026-03）、マトリクスは MAJOR に 95 以上を求める（2026-05）。
  仕様の file:line と該当行の引用を必須にし、引用して diff と突き合わせられる指摘は 95 まで付けてよいとした。「仕様に書かれていない」
  ことしか示せないスコープ逸脱や解釈の割れる指摘は 80 を上限にする（mattpocock/skills の code-review が Spec の指摘ごとに仕様の行を
  引用させる方式の翻案）
- scoring-guide の「報告閾値を割った指摘の記録」に 3 つ目の経路「最初から閾値未満」（反証も加減算も受けず、reviewer の付けた値のまま
  マトリクスを通らない）を足し、両 SKILL のテンプレートに脱落理由の書式を足した。reviewer は severity でしか列挙を絞らないので、
  MAJOR の 60〜94 はこの経路で落ちるが、書式が無かった
...
- **報告 0 件の回に限り、閾値未満の候補件数をレポートに 1 行通知する**（`### 🔎 閾値未満の候補` / GitHub issue #242）。
  閾値未満は reviewer が `## below-threshold` に件数だけ返し、🔁 付録の対象でもないため、「何も見つからなかった回」と
  「見つけたが全部閾値未満だった回」がユーザーから区別できなかった（実測: opus-4-8 層の真の空振り 17 件中 14 件が後者）。
  条件は報告 4 バケツすべて 0 かつ `below_threshold_counts` の合計 1 件以上で、payload に埋める値だけで決まる。
  出すのは件数と severity 内訳だけで、本文の書き起こしはしない（件数だけ返す節約は維持）。payload は変えていないので、
  retro の「真の空振り率」はこの変更では動かない（変わるのはユーザーへの見え方）。規約は scoring-guide.md `### 閾値未満の件数通知`。
...
- **報告件数 4 フィールドの欠測で publish を止める**（GitHub issue #238 候補 (a)）。v2.111.0（#215）は gap `payload:report_counts.missing`
  を立てて通していたが、検知だけでは欠測が止まらず、v2.127.1 の入れ子受理でも救えない「count キーそのものが無い」型が現行版で
  2 件残った。1 つでも欠けると publish はイベントを書かずに exit 1 し、4 キーを揃えて再実行するよう stderr で促す。
  - #203 / #208 が fail-fast にしなかった判断は変えない — あちらは他の分子が生きている。こちらは欠けた回を集計側が既に母集団から
    外している（#212 / #213）ので「通す」は失うものを増やさず、「止める」は区間・トークン・dispatch を再実行で取り戻せる
    （打点ファイルは残る）。理由は publish のコメントに記録
  - `missing` の gap は旧版のイベントにだけ残る識別子になる（retro の読み分けはそのまま）
  - `findings_class` 突合の「件数が揃っている回だけ」の条件を外した（欠測は上で止まるので到達しない）
...
- **comment-polish（B 系統）の適用を A 系統 findings 件数から独立させた**（GitHub issue #236）。従来は Step 7 の修正方針フロー自体が
  「A 系統 findings 1 件以上」でしか走らず、findings 0 件の回は `## コメント推敲提案` が出ていても一度も適用されなかった。
  - Step 7 の実行条件を「A 系統 findings または B 系統提案のいずれかが 1 件以上」に変更
  - AskUserQuestion に B 系統専用の質問（適用する / 適用しない）を追加し、該当する質問だけを 1 回の呼び出しにまとめる
  - 「このまま」（A 系統を修正しない選択）とコメント推敲の適用が独立に選べるようになった
...
- **retro の判定保留行に「あと N 件で判定可能」を出す**（GitHub issue #230）。`--min-plugin-version` で
  版を絞ると、⚠️ のサンプル下限（反証・真の空振り 10 / meta 8 / skeptic 15）に届かず保留が主要な
  出力になるが、件数が無く読み直す時期が決まらなかった。
  - N は「下限 − 層の分母」。届いた時点で閾値を超えているとは限らない（判定が「できる」であって「鳴る」ではない）
  - `--json` に `pending`（signal / layer / numer / denom / min_n / remaining）を追加。テキストの 1 シグナル 2 層切りに依存せず全層を出す
  - `--since` / `--last` / `--min-plugin-version` で絞った実行では、保留行の「累計では」を「絞り込んだ集計全体でも」に変える
  - 下限の値そのものは変えていない（決めること 1 は保留。実測で供給レートが 2 マシン分あり、窓でも数日で下限に届いた）
  - 回帰テスト 3 本
...
- **retro に `--min-plugin-version <版>` を足した — 打ち手の効果を版で絞って読む**（GitHub issue #210）。
  回復の判定基準（真の空振り率 20% 未満）が累計で評価されるため、打ち手より前の崩壊期の回が
  分母に残り続け、率が構造的に下がらなかった（opus-4-8 は 9/30。20% 未満には 46 件以上要る）。
  - 指定した版以上で publish された回だけを集計する（数値比較。`2.10` は `2.10.0` と同じ）。
    `plugin_version` を持たない回は推測で入れず除外し、除外件数を「集計」行と `provenance.filters` に出す
  - `--last` は版で絞った後に掛かるようにした（先に掛けると N 件に満たない）
  - 数字とドット以外の版指定は exit 2（`v2.10.0` / `latest` を 0 に丸めると全件が残る）
  - 真の空振り率の ⚠️ は、絞っていない集計では「回復は打ち手を入れた版以降に絞って読む」を添え、
    絞った集計では絞り込み条件を名乗る
  - **直近 N 件の窓は採らなかった** — retro 冒頭の層別の原則「版で切り、日付では切らない」と、
    #210 で「データに合わせて定義を動かして基準を通す」を却下した判断に反する
  - 回帰テスト 6 本
...
- **`agents-mismatch` の回に隠れていた一括発行違反を数える**（GitHub issue #220）。retro は自己申告の
  体数が機械計測と食い違った回を「期待 wave 数を作れない」として分母からも分子からも外していたが、
  実測で `[1×14]`（14 体を全部別メッセージで発行）と `[1,1,1,1,6,6,1,6]` という最悪の逐次発行 2 件が
  そこに隠れていた。
  - 全層が起動した場合の期待本数（`MAX_EXPECTED_WAVES` = 7）を `lib/wave_expect.py` に置き、
    `agents-mismatch` の回でもこれを超えたら違反と確定する。上限以下の回は従来どおり抑止
  - 当初案の「`verdict: serial` を数える」は採らなかった — 単独 wave の連続だけで決まり期待本数を
    見ないので、体数の少ない正当な構成を拾う余地がある
  - 実データ（gist 集約 + ローカル n=226 / retro を `yukis-MacBook-Pro-2023` で実行）では 2 件が抑止から
    違反へ移る（判定成立 42 → 44 / 違反 9 → 11 / 抑止 5 → 3）
  - 回帰テスト 4 本（上限超過で確定 / ちょうど上限は抑止 / `waves_effective` で比べる / 上限値と式の一致）
...
- **真の空振り ⚠️ の処方を内訳で分岐させた**（GitHub issue #210）。率（報告 0 かつ推奨 0 /
  回復サイン 20% 未満）は**据え置き**で、動かしたのは打ち手の指し先だけ。
  - 検出 0 が主 → 「実行世代を見直す」（recall の問題）
  - 閾値未満のみが主 → 「閾値と付録の方針」を指し、**世代を上げても同じ結果になりうる**と明示。
    従来はこの層にも「実行世代を見直す」と出ており、実測 9 件中 7 件で是正先が誤っていた
  - 同数・判定不能 → 打ち手を 1 つに絞らない（推測で片方へ倒さない）
  - **率の分母を「検出 0」だけに絞る案は採らなかった**: 基準（20% 未満）は満たせるが、
    データに合わせて定義を動かして基準を通す形になり、#217 で閾値の層別調整を却下したのと同型

## [2.117.0] - 2026-09-07

### Added

- **真の空振りの内訳を「検出 0」と「検出はあったが全部閾値未満」に割った**（GitHub issue #210）。
  表・⚠️ シグナル・`--json`（`true_silent_split`）の 3 箇所に出す。**打ち手が違う**ため —
  前者は reviewer が何も見つけていない（recall の問題）、後者は見つけたものが
  `## below-threshold` に件数だけ返り、報告にも付録にも出ない（閾値と付録の方針の問題。
  付録の対象は「reviewer が列挙した指摘」だけという契約 / `scoring-guide.md`）。
  - 実測（opus-4-8 / n=21）: **真の空振り 9 件のうち検出 0 は 2 件だけ**で、残り 7 件は MINOR を
    2〜6 件見つけていた。4.8 は検出が空なのではなく検出の 88%（111/127）を MINOR に置いており、
    閾値 MAJOR の下では設計どおり報告 0 になる（opus-5 は 67%）
  - `pre_adjust_counts` を持たない回・語彙違反の回は `unknown` に置く（0 に丸めると
    「検出が無かった」に化ける）
  - **#210 の回復サイン「20% 未満」がどちらの改善を求めているのかは、この内訳を見てから決める**。
    `triage-guide.md ### 5.2` に「世代を上げる」以外の打ち手があることを記録した
...
- **`review-retro.sh` が synthesis の支配率（`duration_synthesis_min` ÷ `duration_fleet_min`）を出す**
  ...
  - ⚠️: 30% 以上の回が 10% 以上の層で鳴る（`layered_signal` / 下限 10）。**閾値は実データから決めた**
    （n=71: 中央値 5% / 75% 点 15% / 90% 点 35% → 30 は 90% 点付近で 9 件が該当）。初回から
    `unrecorded` 層で鳴るが、#218 が「外れ値の領域」と呼んだ回そのものなので偽陽性ではない
  - synthesis が fleet を超えた回（内数の契約に反する打点の矛盾）は母数から外して件数だけ出す
...
- **同名 command の description に `トリガー:` を複製した**（GitHub issue #206）。command 名と skill 名が
  ...
  ルーティングには効いていなかった。対象: `review` / `self-review`。
...
- **付録（真の空振り）を世代で層別し、真の空振り率を ⚠️ に載せた**（GitHub issue #214）。#210 の
  判定基準「preMAJ 中央値 1 以上 かつ 真の空振り率 20% 未満」のうち、後者だけが機械化されて
  いなかった（表にも出ず、読み手が引き算していた）。#209 の趣旨どおり、**表に出ていても鳴らな
  ければ行動につながらない**。
  - **真の空振り = 報告 0 件かつ付録推奨 0**（#168 の「付録に救われた回は空振りではない」を
    差し引いた値）。`報告 0 件率` は救われた回を含む粗い代理で、こちらが本命
  - 世代が 2 種以上あるときだけ表を割る（`with_gen` と同じ方針）。`--json` に `appendix_by_gen`
  - ⚠️ は #209 の `layered_signal` に乗せた。閾値 20 は #210 本文が「回復のサイン」として先に
    固定した値、下限 10 は反証の不発と同じ。**実測（gist 集約 n=183）で新たに鳴るのは
    opus-4-8 の 1 層だけ**（43% = 9/21。他は下限未満）— 初回から鳴りっぱなしにはならない
  - `orchestration-measurement.md ## 18` に真の空振り率の定義を追記（`## 16` は触っていない）
  - これで #210 の判定基準 2 つが両方 ⚠️ で機械化され、「回復した」を表を読まずに検出できる

## [2.111.0] - 2026-09-05

### Added

- **報告件数 4 フィールドの欠測を publish が検出する**（GitHub issue #215）。`below_threshold_counts` /
  `appendix` / `findings_class` は fail-fast で検証しているのに、**歩留まり・報告 0 件率・真の空振り・
  `findings_class` 突合のすべての分子であるこの 4 つだけ検証が無かった**。1 つでも欠けると
  `payload:report_counts.missing` を立てて stderr に WARN を出す。**fail-fast にしない**（#203 と
  同じ判断 — 止めるとその回の計測が丸ごと消える）。**0 件は欠測ではない**。
  - 実測（gist 集約 n=183）: 欠測 7 件のうち **4 件は現行版**で、LLM がテンプレートを埋め落としている。
    retro の注記が全部「旧版の payload」と説明していたのは誤帰属
  - 欠測で `findings_class` の突合をスキップした事実も同じ識別子で残る（従来は無音）
  - `## 16` の表に「4 つとも必須で 0 件でも省かない」を明記（`below_threshold_counts` と同じ文言）

### Changed

- **報告 0 件率の注記を「旧版」と「現行版の埋め落とし」に分けた**（#215）。除外は正しいが説明が
  誤っていた。現行版の埋め落とし（publish が gap を立てた回）は**サンプルの損失**で、増えるなら
  SKILL 側のテンプレートを直す根拠になる
- **報告 0 件率テーブルも、全件除外で 0 行になったとき注記を出す**（#212 / #213 と同じ穴の
  4 箇所目）。ガードの内側に注記を置くと、除外した結果 0 行になった回は注記ごと消える
- テストの既定 payload（`BASE_PAYLOAD`）に報告件数 4 つを足して契約準拠にした

## [2.110.1] - 2026-09-05

### Fixed

- **検出 → 報告の内訳（#146）にも報告件数欠測のゲートを足した**（GitHub issue #213）。#212 が
  歩留まりと真の空振りの 2 箇所に入れたのと**同型の穴の 3 箇所目**で、同じ 4 フィールドを
  `or 0` で読む箇所はこれが最後。`post` が 0 に化けると `written − post` が全部 `dropped` に
  計上され、**申告していないだけの回が「本文を書いてから捨てた 100%」になる**。
  - 実測（別マシンの gist 集約 n=183）: `mixed` 世代の行がこのアーティファクト**だけ**で
    構成されていた（`本文を書いてから捨てた: 1 件（100.0%）`）。修正後は行ごと消える
  - **隣り合う 2 表の同一層の n が揃った**。#191 のセルフレビューが「同じ粒度にする」と決めた
    歩留まりと内訳が、opus-4-8 で n=21 と n=23 に割れていた（差の 2 件がちょうど欠測の回）。
    回帰テストで一致を表明した
  - 除外件数は出力に残し、**全件除外で 0 行になった回も通知を出す**（#212 のコメントが踏んだ
    「ガードの内側に置くと通知ごと消える」を繰り返さない）

## [2.110.0] - 2026-09-05

### Added

- **Phase 0 で踏み下げ世代を検出したら警告する**（GitHub issue #210 候補 1a）。`triage-signals.sh`
  が `models=` を出すのと同じ場所で、実行世代が **4 系**（`claude-opus-4-8` 等）なら stderr に
  `WARN: ⚠️ 世代:` を 1 行出す。**中止も確認も入れていない** — 回すかどうかは依然として
  利用者の判断で、警告は「81% が報告 0 件になる世代で回す」というコストを選択の瞬間に
  見えるようにするだけ。オーケストレーターはその 1 行を Phase 0 のレポート冒頭に転記する
  （publish の `⚠️ 計測:` と同じ慣例）。
  - **述語は「ベースラインと違う」ではなく 4 系世代。** 「5 系でない」にすると、より新しい
    世代（`fable-5-1` 等）で回した回まで鳴る。混在（`混在（A/B）`）は現在どちらで走っているか
    決められないので鳴らさない。5 系が踏み下げになる日が来たら**黙る側に倒れる**（誤爆しない・
    保守は `case` の 1 パターン）
  - **根拠は #212 の欠測ゲート後の数字**（別マシンの gist 集約 n=183 / 2026-09-05）:
    opus-4-8 で報告 0 件率 **81%**（17/21）・真の空振り **43%**（9/21）・`pre_adjust` MAJOR
    中央値 **0**。計測アーティファクトの寄与は 1 件で、看板の数字は生き残った
  - **不発率は世代の指標にしない**（`unrecorded` は preMAJ 9 でも不発、opus-5 は preMAJ 0 でも
    発火）。警告文に載せるのは preMAJ 中央値と真の空振り率だけ
  - SKILL 本文は触っていない（review/SKILL.md は本文 505 行で skill-size の上限超過）。
    規則の正本は `triage-guide.md` `### 5.2`（SSoT pin の source ではないので打ち直し不要）

## [2.109.1] - 2026-09-04

### Fixed

- **報告件数フィールドの欠測が「報告 0」として集計に入っていた**（GitHub issue #212）。
  `blocker_count` / `critical_count` / `major_count` / `minor_count` を 1 つも持たない回が、
  歩留まりと真の空振りの分子に 0 として計上されていた。**#203 が `pre_adjust_counts` の
  語彙違反について同じ理由で母集団から外したのと同型の穴**で、そちらのコメントが失敗の形を
  そのまま書いている（実数が 0 として分子に入り歩留まりを下振れさせる）。
  - **歩留まり**（`if v:` が `None` を飛ばす）は**ローカルで 3/43 件発火していた**。
    実害の実例: このリポジトリで唯一の opus-4-8 サンプルが、報告件数フィールドを持たない
    だけで **`検出 9 → 報告 0（0.0%）`** として表に出ていた。#210 が「踏み下げた世代で検出が
    死んでいる」と読む材料に、計測アーティファクトが混ざっていたことになる
  - **真の空振り**（`or 0` で欠測が 0 に化ける）は `apx_silent` の分子で、**#210 の看板の
    数字（真の空振り 44%）がこの上に乗っている**。ローカルでは `appendix` を持つ回が 2 件
    しかなくどちらも報告件数を持つため未発火だが、#210 の母集団は別マシンにある
  - 報告 0 件率のテーブル（`all(v is None)` でゲート済み）だけが正しく、**3 箇所のうち
    2 箇所が素通り**という非対称だった
  - **除外した事実は出力に残す**（`zero_missing` と同じ流儀）。黙って外すと母数が減った
    理由が読めなくなる。**全件除外で 0 行になった回も通知を出す** — ガードの内側に置くと
    通知ごと消えて「該当なし」と読まれ、直そうとしている誤読の同型になる
  - **本当に 0 件の回は従来どおり数える**（非退行をテストで表明した）。一緒に落とすと
    recall が落ちた回まで母集団から消えて「歩留まりは健全」に見える

## [2.109.0] - 2026-09-03

### Changed

- **⚠️ シグナルの発火判定を世代の層ごとに行う**（GitHub issue #209）。表は #191 で世代別に
  なったが**判定だけ累計のまま**で、母集団の違う層を平均する形が残っていた。実測では
  反証レイヤーの不発が opus-4-8 で 78% あったのに、累計 43% では閾値の 50 に届かず、
  **9 日 18 レビューの検出崩壊が一度もシグナルに載らなかった**。対象は 3 つ
  （反証レイヤーの不発 / meta の起動ゼロと価値率 / skeptic の価値率）。
  - **閾値を超えた層が 1 つも無ければ累計で評価する。** どの層も下限に届かない期間は
    珍しくなく（既定母集団はまさにその状態）、層別だけにすると**いま鳴っている行が消えて
    「該当なし」と読まれる** — #209 が直そうとした失敗を裏返しで再生産することになる
  - **分岐を「判定できた層の有無」で切らない。** 判定できた層が健全なだけのとき、
    下限に届かない層が崩れていても行が 1 本も出なくなる。このリポジトリでは実際に到達する
    経路で、`unrecorded` 層は現行版が必ず世代を記録するため増えず、伸びるのは最新世代だけ
  - **累計で判定した回は理由を行に添える**（「層別では判定不能 — 最大層 X の分母 N・下限 M」
    または「層別では X が下限に達したが閾値に届かない」）。黙ると層別が効いていると読まれる。
    **分母はシグナルごとに違う**ので注記も同じ関数から取り直す（skeptic の分母は起動回数で
    あって母集団ではない）
  - **下限に届かないが閾値を超えている層は「計測の健全性」に判定の保留として出す。**
    ⚠️ には出さない（下限未満は「まだ行動しない」という判断そのもの）が、黙ると
    「該当なし」と読まれる。**保留にも下限を置く**（単発点灯の防止 / `GAP_MIN_N` と同じ流儀。
    置かないと n=1 の層が毎回 3 本並んだ）
  - `by_gen` に `valuable` を足した（価値率を層別するのに要る）

- **#150 の撤去条件を厳密化した**（#209 と同じ「累計で読むと誤る」の是正）。旧記述は
  「**次の** review サンプルで構成比が下がらなければ戻す」で、**n=1 で規約を撤去できる文面**
  だった。同じ doc が「1 件でも点灯する条件を混ぜると ⚠️ が常時点灯して契約が壊れる」と
  書いているのと食い違う。撤去は ①同一層（`calib` × 世代）内の review サンプルが 5 件
  ②その層で下流の `miscategorized` 構成比が下がっていない ③同じ層で上流も下がっている、の
  3 つを**すべて**満たしたときに限る。
  - **判定は構成比で行い、件数の増減を根拠にしない。** 件数は層の n に依存する
    （実測: 上流の降格は 23 件 → 8 件と**減った**が、`miscategorized` 構成比は 58〜64% →
    88% と**上がっている**。件数で読むと結論が逆になる）
  - **判定不能のときは撤去しない**（「効いていない」ではなく「まだ測れていない」）。
    このリポジトリ単独では review 側のサンプルが構造的に増えないので、`--logs` での
    合算が唯一の解決経路であることも併記した

### Fixed

- **新規テストクラスが基底の 52 本を再実行していた**。`RetroTest` は自前のテストを持つので、
  そこを継承すると子の名前でもう一度走る（同ファイルの `TranscriptFixture` が記録している
  失敗と同型）。fixture だけを持つ `RetroFixture` を切り出し、新規 2 クラスをそちらへ寄せた。
  **既存 6 クラスは同じ形のまま**なので別途対応が要る

## [2.108.0] - 2026-09-03

### Added

- **Phase 0 に実行世代を出す**（GitHub issue #210 の一部）。`triage-signals.sh` の `## meta` に
  `models=` を 1 行足す。踏み下げた世代で回した回は検出が落ちるが（実測: opus-4-8 の
  18 レビューで真の空振り 44% / `pre_adjust` MAJOR 中央値 0）、**それが分かるのは publish 後の
  集計まで待たないといけなかった**。回す前に見えれば、そのレビューにコストを払うかを人が決められる。
  - **警告も中止もしない。** 出すのは事実だけで、どう扱うか（踏み下げた世代で回さない /
    規約を世代で分岐する / 観測を続ける）は **#210 が明示的に利用者側へ残した判断**。
    したがって **#210 は open のまま残す**
  - **`models=` は agent へ渡さない。** 世代を伝えるとレビュー挙動が変わりうるので、
    `## meta` の他のキーとは扱いが違う（SKILL の表に明記した）
  - **`sub` 側は出さない。** Phase 0 では fleet がまだ起動していないので、値が入るのは
    **前回の fleet の世代**であって、これから起動する世代ではない
  - 混在セッションは単一世代に倒さず `混在（A/B）` と出す（#169）
  - **誤値を出さないための縮退が 3 段**（`## 13.1`「縮退先は欠測であって誤値ではない」）。
    ①セッション id が無い ②その id の transcript を引けない ③main 側の実モデル名が
    1 つも引けない — どれも**行ごと出さない**。特に②は **`--session` を省いて `ls -t` の
    最新に倒さない**（同一リポジトリで並行セッションがあると他セッションの世代を出す）
  - 実行コストは transcript 1 本の走査で 0.06 秒（Phase 0 全体で 0.31 秒）
  - 規則の正本は `triage-guide.md` `### 5.2`。**`orchestration-measurement.md ## 16` は
    触っていない** — Phase 0 の表示は payload 契約ではなく、触ると SSoT pin 2 本の
    打ち直しになるため

### Fixed

- `triage-signals.sh` で `CLAUDE_CODE_SESSION_ID` を素で展開していた箇所を既定値つきに直した。
  `set -u` 下では未設定時に unbound variable になり、手前のガードが唯一の防壁になっていた

## [2.107.0] - 2026-09-03

### Added

- **fleet 区間が agent の起動スパンを覆えていない回を欠測に倒す**（GitHub issue #207）。
  `dispatch.span_sec` は transcript 由来の実測（**agent の起動時刻の幅**。issue 本文の
  「最後の終了まで」は実装と食い違うので doc 側は実装に合わせた）なので自己申告ではなく、
  覆えていない回は t1 か t2 の打点が区間の外にある。**汚染は欠測ではなく「もっともらしい
  小さい値」として入る**ので、0 のまま中央値と相関の分母に混ざっていた。
  - 判定は**窓が `since-t0` の回だけ**（`session` は起点に下限が無く、`since-t0-late` は
    締めのあとに起動した agent が窓へ入るので、スパンが区間を超えるのが正常）
  - **丸め 1 分ぶんの余裕を見る**（区間の分は秒を 60 で割って切り捨てるので、真に等しい
    回でも最大 59 秒はみ出す）
  - **倒すのは `duration_fleet_min` だけ**。矛盾が示すのは「区間が起動スパンを覆っていない」
    ことだけで、t1 と t2 のどちらが誤っているかは特定できない。t2 を早く打った回では
    triage と explore は正しいので、そこまで倒すと正しい実測を捨てる
  - **集計側にも同じ判定を持たせた** — publish 側のガードはこれから publish する回にしか
    掛からないが、汚染された値はもう集計に入っている（#199 と同じ理由）
  - **副作用**: `review-backfill.sh` は 3 区間から計測窓を再構成するので、倒した回は後付けの
    対象から外れる（もともと窓が壊れている回なので許容する）
- **`agents` の語彙検証**（GitHub issue #208）。`pre_adjust_counts` と対称で、綴り違いは
  黙って 0 として集計される。`agents` は体数中央値・fleet 相関・1 体あたり cache_read の
  **すべての分母**なので、汚染がそのまま全指標を歪める。
  - **契約は 7 キー**で、体数はうち 5 つ。**突合用の体数 allow-list を語彙検証に流用しない**
    — `explorer_waves` は publish 自身が注入するので、5 キーで判定すると**将来のすべての
    publish が偽陽性**になる（実測: ローカルの判定対象 15 件すべてが該当し 100%。7 キー
    契約なら 2 件 ＝ 13%）
  - 体数のキーが 1 つも無い回は `payload:agents.empty`。集計側の `total_agents` が `None` を
    返して黙って母集団から落とすので、**落ちた事実**を残す
- `--json` に `fleet_span`（`judged` / `conflict`）。判定できた回が少ないうちは比率が閾値に
  届かず ⚠️ が出ないので、**出ないことを「該当なし」と読ませない**ために母数を出す

### Fixed

- **`demoted_types` / `inflated_axes` のトップレベル誤置を「欠測」と呼ばなくなった**
  （GitHub issue #208）。埋める側が `## 16` のテンプレートを見て親の外へ書いた回が実測 5 件
  あり、不在判定は親の中しか見ないため**同じ回が「記録漏れ」として集計され、是正先
  （ネストを戻す）が読めなくなっていた**。別識別子で立てて `gap_hint` を分け、不在 gap とは
  排他にする。**値は救わない**（正しい位置へ移さない） — publish が書き換えると埋める側は
  誤りに気づかないまま同じ誤置を続ける（`missing_coverage` の「黙って正規化はしない」に揃えた）
- **誤置の導線を塞いだ**: 両 SKILL の同じ 1 文で `adversarial_verify.inflated_axes` は親付き
  なのに `demoted_types` だけ親無しだった。`below_threshold_counts.demoted_types` に揃え、
  「親オブジェクトの中に置く」を明記した
- **`fleet` が 0 分の回が体数相関の分母から落ちうる境界**（変異テストで検出）。0 分は実測で
  あって欠測ではない。落とすと「速い回」だけが構造的に相関から消える

### Changed

- `orchestration-measurement.md` `## 16` の `measurement_gaps` 語彙一覧に新識別子 5 つを追記し、
  **#211 が残した宿題も同じ行で回収した**（`agents-abandoned` / `agents-nested` が語彙一覧に
  無かった）。payload テンプレートの注意書きと `agents` の契約キーも更新。両 SKILL の
  SSoT pin を打ち直した（差分は pin 2 行のみで巻き込みなし）
...
- **`agents_dict` の docstring が変異テストで必ず生存し、CI（`--strict`）を落としていた**。
  `.sh` 内の python ヒアドキュメントは `#` で始まらないため変異ツールの除外に掛からず、
  説明文中に書いた演算子の字面がそのまま書き換えられる。CLAUDE.md Gotchas が比較演算子
  （`>=` / `<=` / `<` / `>`）について警告している当の落とし穴の、**論理演算子版**
  （`or` → `and`）。挙動が変わらないので必ず生存する
  - 演算子の字面を使わない書き方に直し、**同じ穴を次に踏まないための警告を docstring
    自身に書いた**（行内コードに入れても効かない点も明記）

## [2.105.0] - 2026-09-01

### Added

- **反証レイヤーの対象帯の外だった指摘に、指摘単位のマーカーを付けるようにした**
  （GitHub issue #196）。既定 high のゲートは BLOCKER 60-94 / CRITICAL 80-94 限定なので、
  **MAJOR は層が動いた回でも一度も検証されていない**のに、レポート冒頭の
  `反証: 対象 N 件 …` の 1 行だと帯の外にいた指摘まで検証済みに読まれていた。
  指摘を 1 件ずつ採用する経路では冒頭の 1 行は参照されない
  - 記法は `⚠️ 反証未実施（対象帯外）` の本文形式。**`[...]` タグ語彙は増やさない**
    （`[...]` は reviewer 自己申告の系統で、これはオーケストレーターが付ける）
  - **`fired=false` の回には付けない** — 全指摘が対象になり 100% に付くので
    「⚠️ が出たときだけ行動する」契約が壊れる。その回は既存の
    `反証: 未実施（対象帯に該当なし…）` の 1 行が担う（#136）
  - スコアは動かさない（帯の外にいたこと自体は指摘の質の情報ではない）

### Changed

- **反証レイヤーが覆っていない指摘では、採用時の一次ソース確認を load-bearing 判定より
  前に置くようにした**（#196 期待動作 3）。訂正の伝播前ガード（#71）は「load-bearing な
  事実主張」に絞って確認を求めるが、**独立検証を一度も通っていない指摘では
  *どれが load-bearing か* の判定自体が指摘を書いた同じ 1 体の推論に乗っている**。
  実測でこの契約がある状態のまま失敗している（2026-08-25）
- `scoring-guide.md` の「verdict が付いていない指摘は no-op」に、**no-op なのは
  confidence 計算だけで「検証済み」を意味しない**ことを明記した
- **反証ゲートの帯は今は広げない**と判断し、根拠を `design-notes/scoring-rationale.md`
  に記録した（#196 期待動作 2）。不発率のシグナルは既に鳴っており、かつ世代と交絡して
  いる（#191）ため、原因の切り分け前に打ち手を当てない。再検討の条件も併記
...
- **往復上限（#156 Phase 2 / GitHub issue #190）の判定基準を A/B の前に確定させ、
  設計ノートに記録した**（`design-notes/pending-optimizations.md ## 11`）。**上限そのものは
  まだ入れていない** — issue の明示条件（recall への影響測定とセットでしか入れない）に従う
  - `cache_read` 対 往復回数の相関 r = 0.978 を**独立な run で再現**（reviewer 5 + explorer 2 /
    再試行なし）。ロール別の往復単価差も再現した（reviewer 106〜138k / explorer 65〜82k）
  - 採用条件を「`cache_read` が 20% 以上下がり、かつ MAJOR 以上が 1 件も減らない」に固定。
    20% は cap 50 での実測削減 18% の上に置いた（これを下回ると cap が噛まない設定でも通る）
  - 前提条件として「2 群を同一世代で揃える」「両群 MAJOR 0 件なら判定不能」を明記（#191 の
    コメントが指摘した、指標が死んでいると採用判定が自動で通る穴を塞ぐ）

## [2.103.0] - 2026-09-01

### Fixed

- **動的層の発火率と skip 理由を世代で層別するようにした**（GitHub issue #191 期待動作 1 の
  積み残し）。コスト側（fleet / 体数 / cache_read）と検出側（歩留まり / 報告 0 件率 /
  反証 verdict）は v2.95.0 で世代キーが入ったが、**`layer_stats` だけが全世代の累計**のまま
  残っていた。踏み下げの崩壊はここに最も鋭く出る — 上流の MAJOR がほぼゼロになると反証は
  `no-eligible-findings` で不発になり（実測: calib=3 の 7 件中 6 件）、累計すると
  「反証 7/13 ＝ 54%」に見えて**ゲート幅が広すぎる**という別の是正先を指す。世代別に割ると
  4.8 が 14%（`no-eligible-findings` 6 件）/ opus-5 が 100% と分離される
  - 内訳は `GEN_SPLIT` かつ**その層に 2 世代以上届いているとき**だけ出す（層のフィールドを
    持たない旧版 payload は層の母集団に入らないので、`GEN_SPLIT` だけを条件にすると本体の
    1 行と同じ内容が 2 度並ぶ）
  - `--json` の各層オブジェクトに `by_gen` を追加（分母は本体の `n` と一致する）

## [2.102.0] - 2026-09-01

### Fixed

- **層別表 2 つ（`effort × size_tier` / `1 体あたり cache_read`）の表示上限が silent に切って
  いたのをやめた**（GitHub issue #198）。上位 8 層で切りながら**落とした層数も件数もどこにも
  出しておらず**、「どのログから何件採ったかは必ず出力する」という本スクリプトの流儀（冒頭）に
  この 2 表だけが従っていなかった（同ファイルの `gap_hits[:2]` は上限の理由と補償を明記済み）
  - **省略行を出す**: `他 N 層（計 M 件）を省略（表示は n の多い順 上限 8 層…）`。0 件のときは
    出さない（常時出ると読み飛ばされる）
  - **世代ごとに最低 1 行残す**: 切ると落ちるのは必ず n の小さい層 ＝ **新しい世代**なので、
    #191 で世代キーを足した結果、世代交絡を消すための層別が新世代の観測を消していた
    （実測 n=108 で opus-5 が 2 表とも 0 行 / #190 の A/B の前提も壊れる）。上限を超えても
    各世代の最大の層 1 つは救う
  - 層別キーから世代を割り戻さず `with_gen` が逆引きを記録する（`effort` / `size_tier` は
    payload 由来の自由文で `/` を含みうるため）
...
- **返答・承認メッセージの締めから「認識に誤りがあればご指摘ください。」を廃止した**。
  結論が出ている報告（2.1 完全対応 / 2.2 部分対応 / 2.3 据置）は締めの一文を置かず、
  対応内容・理由で終える。誤りがあればレビュアーは再度コメントできるので、それを促す一文は
  文面を伸ばすだけになる。「いかがでしょうか」系（応答を求める形）は従来どおり
  **相手の応答がないと次に進めない** 2.4 / 2.5 / 残存指摘のある 2.6 に限る
- **投稿する文面で良い点に触れないようにした**（reply-tone-guide `0.4` を「褒める」から
  「触れない」へ反転）。承認メッセージの良い点 1 文・著者配慮チェックリストの該当項目・
  closing-flow-guide の「良かった点」圧縮を削除した。文面が伸びて著者の読む量が増えるため。
  **著者への配慮（0.2 敬意 / 0.6 チェックリスト）は据え置き** — 称賛を落としても配慮は落とさない
- レビューレポート本体の「良かった点」セクション（`skills/review/SKILL.md`）は
  **チャット出力なので存置**。投稿文面への転記だけを禁じた
...
- **`wave-split` の分母・呼称・是正先を直した**（GitHub issue #192）。3 点とも誤っていた:
  - **分母**: 他の gap は「フィールドが載っていたか」なので `measurement_gaps` 保持者
    全体でよいが、`wave-split` は**wave 判定が成立した回**が母集団。全体を分母にすると
    比率が薄まり、実測 50% が 18% に見えて ⚠️ の閾値を下回っていた
  - **呼称**: 「計測マーカーの欠測」と呼んでいたが、これは**規約違反であって欠測ではない**
    （計測は取れている）。同じ語で呼ぶと是正先を誤読させる
  - **是正先**: `gap_hint()` に分岐が無く既定の「打点箇所の見直し」に落ちていた。
    正しくは「同一フェーズの agent を 1 メッセージで一括発行する」で、打点とは無関係
- **`agents-mismatch` で wave 判定を抑止された回を出力に明示した**（#154 のコメントが
  求めていた処理。分母にも分子にも入らないため、黙って落とすと「守られている」と読まれる）

### Added

- 上流較正の待ち行に**達成不能の警告**を足した（#191 期待動作 2）。対策後の層が 1 世代に
  しか存在しないとき、比率が動いても打ち手の効果と世代の副作用を分離できない。
  そのまま「蓄積中」と出すと達成不能な条件を待ち続けることになる

## [2.95.0] - 2026-08-29

### Added

- **retro の検出・報告側に世代キーを足した**（GitHub issue #191 期待動作 1）。#169 は世代を
  **コスト側だけ**（fleet / 体数 / 1 体あたり `cache_read`）に入れたため、検出・報告側は
  世代をまたいで累計されていた。**「安くなった」だけが見えて「効かなくなった」が見えない**
  非対称になっていた。層別を入れたのは 3 箇所:
  - `pre_adjust` → 報告の**歩留まり**（版マーカー × 閾値 × 世代）
  - **反証 verdict 分布**（`calibration_schema` × 世代）。calib と世代が完全交絡している
    母集団があり、層別しないと打ち手の効果と世代の副作用を分離できない
  - **報告 0 件率**（世代別・新設）。recall の最も粗い代理指標。**母数は報告件数フィールド
    （`*_count`）を実際に持つ回に限る** — 版マーカーだけで濾すと、フィールドを 1 つも持たない
    旧版の回が 0 と評価され**欠測が「報告 0 件」に化ける**（自己レビューで実測: このリポジトリの
    実データでは陽性セルの 3/3 が欠測由来だった）。0 件率は構造的に上振れし、しかも古い版ほど
    欠測が多いので「新しい世代ほど recall が落ちた」を機構が自分で作り出す。除外した件数は
    表に併記する。世代が 1 種でも表は出す（平常状態で指標が消えると劣化を見る手段が無い）

### Fixed

- 反証 verdict の層キーが複合文字列になったことで壊れる下流を直した。閾値比較は層が持つ
  `calib` の数値で行う（文字列との比較は Python 3 では TypeError）
- **シグナル判定を該当する全層で行うようにした。** 層キーに世代が入って同じ calib に複数層が
  並ぶようになったため、代表 1 層だけを見る従来の構造では**条件を満たした別世代が黙って落ちる**
  （「⚠️ が出たときだけ行動する」契約では「該当なし」と読まれる）
- 内訳テーブル（`## 5.1`）も歩留まりと同じ粒度で層別するようにした。片方だけ割ると、
  隣り合う 2 表のどの行を分解した数字なのかが対応付かない
- 見出しの「× 世代で層別」は**実際にキーを割ったときだけ**出す（世代が 1 種の母集団では
  `with_gen` がキーを割らないため、注記と中身が矛盾していた）
- `pre_adjust` MAJOR 中央値の列に**その列自身の分母**を併記した（`major` が欠測の回があり、
  行の n とは異なる）
...
- **`plugin.json` の description を 2177 → 196 字に圧縮した**（GitHub issue #183 /
...
読んだものだけを書くと**「これが全部」と読まれる**。実測ではプラグインを開発しているリポジトリが最も母数を持たず（素の実行で n=2 / `--logs` 合算で n=99）、出力が「サンプル待ち」で埋まったため**判定可能なデータがあるのに 2 セッション判断が先送りされた**。同じ誤読を本セッションでも踏んでいる（issue コメントの「別マシン待ち」を現在値として信じ、集計を回さずに「判定不能」と結論した）。

### Added

- **自動探索の回に母集団の範囲を出す**。`**母集団**: ログ N 本 …（**このリポジトリのログのみ**）` に加えて、**合算コマンドとパス一覧の作り方**を注記で添える（毎セッション `find` を組み直していた / #150 本文・#160 本文・#173 本文で 3 回）
- **0 件の回にも出す**。ここが最も誤読されやすい（「まだ回っていない」と読まれる）
- **JSON に `sources_scope`**（`this-repo` / `explicit`）

### 補足

- **探索は既定にしない**（#160 で「探索範囲が暗黙になる」として `--all-repos` を見送った判断を維持）。消すのは誤読だけで、範囲は利用者が決める
- **閾値は置かない** — 「n が小さいときだけ出す」にすると閾値の下で黙る区間ができ、**そこがまさに誤読の起きる帯**になる。件数が多くても他リポジトリを含まないことは変わらないので常に出す。`--logs` 指定時は利用者が範囲を決めているので出さない（ノイズにしない）
- 期待動作 2（`--logs` のパス集合の永続化）は**未実施**。摩擦の解消は任意要件で、状態ファイルを増やす前に注記だけで足りるかを見る
...
- **v2.86.0 の「回帰テスト 16 件」を実測値 19 件に訂正**。セッションレポート作成時に git 差分で数え直して発覚。**記録された数字の正確さがこの一連の変更（#167 / #168 / #169）の主題そのもの**なので直す

## [2.88.0] - 2026-08-25

**「報告 0 件」と「価値 0」が payload で区別できなかった**（GitHub issue #168）。

実測（`2026-08-24T02:34:04Z` / PR 398 / xhigh / large / 21 体 / 62 分 / sub cache_read 164.9M）は severity 4 バケツすべて 0 だが、レポート本体は 🔁 付録に 18 件を列挙し、**うち 4 件を「この diff 自身のファイルに 1 行で閉じるので見送るのは惜しい」と名指しで人間に推している**。1 件は反証エージェントが `length === 16` を実際に削って `vitest` を走らせ 3314 tests 全緑を確認しており、**テストの穴が実証済み**だった。集計側はこれを「報告 0 件」と読むため、この回は空振りと同じ形になる。実際には空振りではない。

**費用対効果の分子が構造的に欠けている**ので、体数キャップ・effort profile・閾値のどの打ち手も過小評価に倒れる。

### Added

- **🔁 付録の項目単位マーカー `※ 推奨: <理由>`**（正本: `scoring-guide.md ## 報告閾値を割った指摘の記録`）。閾値の外に落ちたが人間には見てほしい指摘に付ける
  - **判断軸は「修正コストが小さいか」**。閾値を割った理由（confidence / severity）は既に脱落理由の欄にあるので繰り返さない
  - **総括に再掲しない**。同じ事実を 2 箇所に書くと drift する（この repo が SSoT pin で繰り返し収束させてきた型）
- **payload に `appendix`**（`{listed, recommended}` + `schema`）。`listed` は付録に並べた行数、`recommended` は `※ 推奨:` を付けた行数。0 件でもキーを省かない
  - **この値は LLM の自己申告**（レポート本文の判断そのもの）で、機械計測へ寄せる経路が無い。検証できるのは構造の不変条件だけで、`recommended` が `listed` を超えると publish が fail-fast する。契約表に自己申告であることを明記した（#169 の `models`「自己申告は無い」と対になる）
- **`review-retro.sh` に「報告 0 件のうち推奨ありの回」と推奨率**。**上限は設けない** — 全件が推奨に膨らむ失敗モードは推奨率が先に表に出る。閾値は実測で決める流儀に従った（上限を先に置くと「惜しい指摘が上限を超えたとき silent に落ちる」ことになり、`削ったら痕跡を必ず残す` の系列にも反する）

### Fixed

- **retro が壊れた `appendix` を母数に入れないようにした**。publish の fail-fast は**自マシンの publish 経路にしか掛からない**が、retro は `--logs` で他マシンのログも合算する（#160）ので、検証を通っていない値が来うる。潰すと欠けた値が 0 件として推奨率の分母に混ざる（変異テストで検出）

### 補足

- 回帰テスト 11 件追加（変異 5/5 killed）。bool が `int` の派生として素通りする経路・非整数で `TypeError` に落ちる経路も含む
- 既存の review は `appendix` を持たないので `measurement_gaps` に `payload:appendix` が立つ。**「推した指摘が無かった」ではなく「フィールドが載っていない」**として retro が区別する
...
集計は `effort` / `size_tier` / `reviewer_effort_profile` で層別する設計だが、Opus 5 と 4.8 が混在した瞬間にこの層別が成立しない。実測では **2026-08-24 の 1 日で 3 サンプル中 2 件が 4.8** で、1 体あたり cache_read の 7,853k と 3,7xx k の差が **tier と世代で完全に交絡**していた（#156 の「深さのコストが下がった」と「4.8 だから軽い」が分離できない）。世代は事故ではなく**ユーザーが実行時に選ぶもの**（エイリアスは親世代を継ぐ / #170）なので、層別キーとして扱う。

### Added

- **payload に `models`**（`{schema, main, main_distinct, sub_distinct}`）。transcript の `message.model` からの機械計測で**自己申告は無い**（#154 の教訓）。`tokens` とは独立に載せる（窓が空振りしても世代は拾えることがある / `dispatch` と同じ流儀）。引けなかった回は `measurement_gaps` に `models`
  - **`main` は単一世代の回にだけ入る**。実行中に切り替えた回（実測: opus-5 → opus-4-8 → opus-5）と引き当て失敗はどちらも `null` で、集計側が mixed の別バケツへ落とす。**片方を代表値に選ばない** — 選ぶと交絡サンプルが単一世代の分布に混ざり、分離が永久に不可能になる
  - **`<synthetic>` は数えない**。実データ 160 transcript の走査で 11 件あり、素通しすると**単一世代の回まで `main_distinct` が 2 件になり mixed に落ちる**
  - **`sub_distinct` の複数値は混在の証拠ではない**。同一レビューで opus と sonnet が並ぶのはロール別ルーティングの設計どおりで、このフィールドは「エイリアスが親世代を継ぐ」前提が崩れた回（`CLAUDE_CODE_SUBAGENT_MODEL` が立っていた等）を事後に検出するための証拠
- **`review-retro.sh` の層別キーに世代を追加**（fleet 時間・体数 / severity_inflated の型別内訳 / 1 体あたり cache_read の 3 箇所）。**母集団に 2 種以上あるときだけキーへ足す** — 1 種しか無い期間まで割ると既存の `effort/size_tier` バケツが n=1 に砕けて中央値が読めなくなる。分割の目的は交絡を切ることであってキーを増やすことではない。旧版（`models` 無し）は `unrecorded` で、**既知の世代と同じバケツに入れない**（「たぶん opus-5 だった」は観測ではない）
- **`review-backfill.sh` に世代列**と、1 体あたり cache_read の世代別中央値。**全行が mixed の回でも内訳を出す** — 1 種だからと畳むと、上に出ている中央値が丸ごと比較に使えない行から出来ていることが表に出ない
- **呼び出し側が渡した `models` は捨てる**（fail-closed）。`models` は「自己申告が無い」ことが値の意味そのものなので、残すと transcript を引けなかった回に **LLM が書いた世代が機械計測のふりをして残る**（#154 の型）。両 SKILL の「スクリプトが注入するので渡さない」列挙にも追加
- **回帰テスト 19 件**（変異テスト 17/17 killed / 生存 0%）。transcript fixture が `message.model` を持っていなかったので既定で載せるよう直した — 落としたままだと publish が毎回 `models` 欠測になり、**欠測が既定の形としてテストに焼き付く**。混在・`<synthetic>`・sub 0 体・空文字・旧版の 5 経路を明示的に作る

## [2.85.3] - 2026-08-24

`review-backfill.sh`（v2.83.0 / v2.85.0）の検証不足の解消。CI の変異スモークと nightly（#165）が**生存 13 件をすべてこのスクリプトで**検出していた。

### Added

- **回帰テスト 9 件 → 26 件**。原因は systemic で、**テストが全部 `--json` 経由**だったこと。既定の人間向け出力（集計行・表）が主インターフェースなのに一度も assert しておらず、突合の一致数・中央値・ギャップ内訳の抽出条件が丸ごと未検証だった。加えて `--projects` はテストを書かずに足していた
  - 既定の探索経路（`--logs` 省略時）/ ログ不在時の `exit 0` / 値を取るフラグ 2 つの引数ガード
  - agent の**起動時刻は先頭行**（末尾を採ると窓の判定が終了時刻になり、窓内で起動した回が「transcript 無し」に化ける）
  - dedup が **payload のキー順に依存しない**（複数ログ合算で母数が水増しされる。実測で ad-hoc 集計が n を倍にした）
  - 窓の**両端**と混入判定帯の境界（片側だけ書くともう片方の境界変異が生き残る。`subTest` で両端を回す形にした）
  - 出力の値まで見る（行の存在だけだと既定値の潰し方の変異が「行は出るが 0」で生き残る）

### Fixed

- **並び順のテストが空振りしていた**。2 件目のイベントの窓が transcript を外れて **1 行しか出ておらず**、「ts 昇順である」の表明が `[x] == sorted([x])` に退化していた。前提（2 件とも窓を作れている）を先に assert する形に直した

### Changed

- 到達不能な 2 行に `# mutation-ok:` を付けた（`inter_wave_agent_sec` / `inter_wave_idle_sec` は `measure-tokens.sh` が必ず同時に入れるので、片方だけ在る dispatch は生成経路が無い）
...
- **`orchestration-guide.md ## 0`（一括発行規約の正本）が検出手段を `dispatch.verdict == "serial"` だけと書いたままだった**。v2.84.0 でその不足（3 連続を要求するため「reviewer 5 体のうち 1 体だけ先に出した」型を取り逃す）を見つけて `wave-split` を足したのに、**規約の本体側が古い保証を約束し続けていた**。3 経路（`serial` / `explorer_waves` / `wave-split`）の射程と、どれも事後計測で実行中には止まらないことを明記した

## [2.85.0] - 2026-08-23

`wave-split`（v2.84.0）の発生率を**サンプル待ちせずに**判定できるようにした。後付けツールを作った当のセッションで、新しい信号だけまた待ちにしていたのを回収した。

### Added

- **`review-backfill.sh` が `waves_expected` / `wave_split` を出す**。式の正本は `publish-review-event.sh` で、こちらは後付け用の複製。**両実装の一致は回帰テストで縛る**（同セッション中に `declared` を意味から再構成して偽の食い違いを出した実例があるため、publish が書いたイベントをそのまま後付けに読ませて突合する）
  - このマシンでの初回判定: **判定可能 6 件 / 検出 2 件 / 偽陽性 0**。うち 1 件（`[1,1,5,3]` の reviewer 分割）は既存の 2 経路（`verdict: serial` / `explorer_waves`）が両方とも取り逃していた回
  - ただし skeptic の fallback 起動は payload から識別できないので、**「偽陽性 0」は fallback 経路が試されたことを意味しない**。WARN 化はまだ保留
- **`--projects`**（transcript の探索先を明示）。`--logs` で他リポジトリのイベントを混ぜても transcript は別 slug にあるため既定では解決できない。**暗黙に全 slug へ広げない** — 同時刻に別リポジトリでレビューが走っていると窓が重なった別セッションを掴みうる。なお本マシンでは n は増えなかった（他リポジトリの 33 件は transcript ではなく**区間欠測**で落ちている）
- **回帰テスト 3 件**（式の一致 / `end` 欠測 / 上記の付随）

### Fixed

- **末尾 wave の `end` が欠測した回に `TypeError` で途中まで出力して落ちていた**（v2.83.0 で混入）。`end` は wave 内に終了時刻を取れない体が 1 体でもいれば `None` になる（`measure-tokens.sh` の縮退 / #153）。実データが全部埋まっていたため表に出ておらず、**式の一致を見る結合テストを書いて初めて踏んだ**
