...
- **回帰テスト 16 件**（変異テスト 17/17 killed / 生存 0%）。transcript fixture が `message.model` を持っていなかったので既定で載せるよう直した — 落としたままだと publish が毎回 `models` 欠測になり、**欠測が既定の形としてテストに焼き付く**。混在・`<synthetic>`・sub 0 体・空文字・旧版の 5 経路を明示的に作る
