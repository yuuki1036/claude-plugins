...
- **コードコメント種別の自動判定（自動ルーティング）**。`--tone code-comment` の明示なしでも、対象の内容シグナルからコードコメントを検出して校正ルートを自動で切り替える（SKILL.md ステップ 1）。
  - **自動検出シグナル（軽量な行構造判定）** — 行頭のコメント記号（`//` `#` `--` `;` `/*` `*` `///` `/**` `<!--`）や `"""`/`'''` docstring、ソース拡張子（`.ts`/`.py`/`.go` 等）でコメント行に閉じた対象。行頭位置で判定し、語中・URL 内の `//`・文中の `#hashtag` では発火しない。ファイル全体を言語別構文でパースする重い処理はしない（0.7.0 の設計判断を踏襲）。
  - **誤爆ガード（fail-safe）** — 確信が持てない・散文とコメントが半々なら**散文デフォルトに倒す**。散文をコメント削除ルートに誤爆させる損失（情報削除・不可逆）は、コメントを散文ルートで処理する取りこぼし（効果が薄いだけ）より重い、という非対称を優先する。Markdown の `#` 見出し・`<!-- -->` は散文扱い。自動で決められないときだけ 1 行で確認する（過剰に聞かない）。
  - **混在対象** — ソースファイル全体などコメントとコードが混在する対象では、コメント行だけを code-comment ルートで扱い、コード本体は対象外にする。

## [0.7.0] - 2026-07-12

### Added

- **コードコメント種別の校正分岐**を追加（`references/code-comment-guide.md` 新設）。散文（PR / Issue / コミット等）とは処理を分け、コメント固有の観点だけを別正本にした。中核は語句の言い換えではなく「このコメントは要るか（残す / 凝縮 / 削除）」の判定。
  - **観点 A: what コメント検出** — コメントが被コメント行の機械的な言い換えに閉じているかを判定し、コード逐語訳（`i++ // i をインクリメント`）を削除候補にする。従来の散文カテゴリには存在しなかった最重要観点。
  - **発火条件を厳格化（fail-closed）** — 削除候補は「コード行と 1:1 対応・why/制約/gotcha/意図を一切含まない・保全 subtype でない」を全て満たすときのみ。一つでも欠けるか判断が割れれば保全側に倒す。`// 昇順でソート（後段の二分探索が前提）` のような「what に見えて why を含む」コメントの誤削除を防ぐ。
  - **subtype 別の保全境界** — docstring / JSDoc（API 契約なので what 正当）・TODO / FIXME・ライセンスヘッダ・区切りコメント・コメントアウトされたコードを保全対象として分離。
  - **責務分離** — 判定は「コメント単体の自明性」に限定し、「コメントとコードの矛盾」（正しさ検証）には踏み込まない（code-review の領分）。宣言でなく判定手順で線引き。
  - **提示側の安全弁** — コメント削除提案は不可逆度が高い（情報損失 + 実ファイル変更）ため常に `[任意][要確認]`、保全明示とセット。新アクションは足さず既存採否フローで「修正案＝削除」として表現。ただし `[任意][要確認]` は presentation-guide の low/medium 抑制対象なので、コメント削除だけは**抑制の例外**として既定 effort でも出す（抑制したままだと what 削除が既定で空振りするため。両ファイルに例外を明記）。
  - SKILL.md に種別判定を追加（発火は「コメント断片直接指定 / Edit 対象行がコメント / `--tone code-comment`」に限定。言語別コメント構文の汎用パースはしない）。コードコメント種別では textlint を skip（散文向けで誤検知が多い）、embed 経路では非発火（委譲元は散文成果物のため）。
  - tone-guide の「唯一の判断基準」宣言と文書種別表を更新し、コメント種別は code-comment-guide を主正本にする旨を明記。
  - 設計は design-review（minimal / clean / pragmatic / risk 4 視点）の findings 9 件を反映して確定。
