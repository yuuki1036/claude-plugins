...
### Fixed
- **stale 判定の日数が実行時刻で 1 日ずれることがあった**（`stale-check.sh`）。BSD の `date -j -f "%Y-%m-%d"` は指定の無い時刻フィールドに**現在時刻**を埋めるため、`now` を取った直後に評価されると差が「N 日 − 数秒」になり、切り捨てで N-1 日に落ちる。閾値ぴったりの doc が**実行した時刻次第で stale になったりならなかったりする**（実測: 境界テストが「20日 > 15日」を期待して「19日 > 15日」を得た）。両辺を日境界（00:00:00）に揃えて age を必ず整数日にした。閾値ぴったり ±1 日を 2 回測る回帰テストつき
