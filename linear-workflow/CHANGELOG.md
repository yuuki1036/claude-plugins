...
### Added
- **issue-maintain に未import knowledge の検知→vault 反映促しを追加**（issue #86）。knowledge の切り出しはこのスキルの責務だが、横断 vault へ反映する `/import-knowledge` は手動トリガーで忘れがち。切り出し完了直後（処理フロー 11.5）に未import件数を検知し、N>0 なら `/import-knowledge {slug}` を最終レポートで促す:
