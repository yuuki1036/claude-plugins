...
| `last-validated` | 任意 | 内容検証日 `YYYY-MM-DD`。記入時は `/knowledge-lint` stale 判定の第一基準（未記入時は `updated` → `verified` を fallback）。doc-freshness プラグインと共通スキーマ |
...
3. type が `investigation` の場合は発火しない（実装を伴わない調査 Issue のため）
